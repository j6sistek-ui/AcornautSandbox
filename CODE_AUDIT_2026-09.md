...
| F95 | `save.ts:405` | The other half of F9, reported from the game (owner, 8 Sep 2026: "i still have to collect acornut everytime i load in"). AcorNut's id was stripped out of `unlockedSuits` unconditionally on every load, so the entry the shelf's COLLECT REWARD tap writes never survived a launch: the card came back unowned and asked for the same collection forever. The strip exists to refuse a grant nobody earned, and it could not ask that question, because `suitRevealed` answers yes partly BECAUSE the id is in the list. `tutorialSuitEarned` asks it without the list — bought, 570 stars, or the beta — and the strip and the graduation lock both read it. An unearned entry is still stripped. |
