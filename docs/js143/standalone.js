...
export async function bootStandalone(root) {
    ...
    function shopCycle() {
        ...
        const suits = dealFrom(suitPool, SHOP_CYCLE.suits, day * 7 + 1);
        // a set already dealt as a suit must not come back as a helmet - they
        // are the same purchase
        const helms = dealFrom(helmPool.filter((i) => !suits.includes(i)), SHOP_CYCLE.helms, day * 13 + 5);
    ...
    function comboOf(cy) {
        const ids = [...new Set([tryOn.suit, tryOn.helm].filter(Boolean))]
    ...
    function drawShopBeta() {
        ...
        if (!tryOn.suit || (!shelved.includes(tryOn.suit) && !cy.held.has(tryOn.suit) && !cy.owns(tryOn.suit))) {
            tryOn = {
                suit: cy.suits[0] ?? heldList.find((i) => SUITS.some((u) => u.id === i)) ?? SUITS[0].id,
                helm: cy.helms[0] ?? tryOn.helm,
        ...
        if (ctx) {
            ...
            const tick = () => {
                ...
                if (palDef)
                    paintPalPreview(ctx, engine.art, palDef.id, 250, 64, 46);
        ...
        if (!combo.ids.length) {
            ...
            go.onclick = () => { packOpen = cy.feature?.id ?? null; confirmBuy = false; render(); };
            bar.append(t, go);
        }
        else {
            const t = el("span", "ac-combotxt");
            const what = combo.ids.length > 1 ? "THIS COMBO" : "THIS SET";
            t.append(el("b", "", `BUY ${what}`));
            const line = combo.trails.length
                ? `Suit, matching helmet and ${TRAILS.find((x) => x.id === combo.trails[0])?.name ?? "its trail"} — the trail is free.`
        ...
        if (cy.feature) {
            ...
            for (const it of bn.items.slice(0, 4)) {
                const u = it.kind === "suit" && SUITS.find((x) => x.id === it.id);
                const h = it.kind === "helm" && HELMETS.find((x) => x.id === it.id);
                if (u)
                    strip.append(suitCardOf(u, 44));
                else if (h)
                    strip.append(helmCardOf(h, 44));
            }
            card.append(strip);
            const txt = el("div", "ac-modtxt");
