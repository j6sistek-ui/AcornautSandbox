import { createVanguardMotion, stepVanguard, vanguardTap, vanguardDive, vanguardContact, vanguardGate } from "./vanguard.js?v=293";
import { PAINTED_TAP_SUITS, TAP_REPEAT } from "./control-constants.js?v=293";
...
export function repeatTapMode(id, save) {
    if (IS_BETA) {
        ...
        if (dialled)
            return dialled;
        if (save.tapRewind && PAINTED_TAP_SUITS.has(id))
            return "rewind";
    }
...
import { createArcflashMotion, stepArcflash, arcflashTap, arcflashDive, arcflashContact } from "./arcflash-motion.js?v=293";
import { createHighOrbitMotion, createPremiumWake, stepPremiumWake, stepHighOrbit, highOrbitTap } from "./high-orbit-motion.js?v=293";
import { isHighOrbit, isPremiumSuit, highOrbitTrailSuit } from "./high-orbit-config.js?v=293";
import { trailWornBy, STAR_CHART_TRAILS } from "./catalog.js?v=293";
import { missionRandom } from "./mission-rng.js?v=293";
import { recordZoneVisit, routeMasks, settleMissionCredit, earnedCampaignStars, migrateCampaign, barrierId } from "./campaign-progress.js?v=293";
import { CHART_LEVELS, reachedGate } from "./campaign.js?v=293";
import { TUNNEL_LEAD_NODES, TUNNEL_LEAD_BLEND, BOUNCE_ANIM_DURATION, LEGACY_DEBRIS_COUNT, ENVS, ENV_GATES, IS_BETA, RETRO_GATE, TAIL, WARP_GATES, TAP_ANIM_DURATION, TUT_READ, PHYS, TRAILS } from "./catalog.js?v=293";
import { nextFamilyPlanet } from "./planet-family.js?v=293";
import { modsUnlocked, batteryUnlocked, writeSave, tailSpringFor, grantTutorialKit, equippedPals } from "./save.js?v=293";
import { platform } from "./platform.js?v=293";
import { TUTORIAL_SUIT } from "./catalog.js?v=293";
import { emptyStats, goalMet, goldGatesFor, gateClearedBy } from "./campaign.js?v=293";
import { createRaceState, RACE_DT, queueRaceInput, raceDecisionAge, stepRace, } from "./race.js?v=293";
import { raceViewport, raceViewportY } from "./race-viewport.js?v=293";
import { createSpill, resizeSpill, spillBurst, spillCleared, spillTap, stepSpill, } from "./spill.js?v=293";
import { SPILL_UTILITIES, spillEngineColor } from "./spill-content.js?v=293";
import { WORMHOLE_MAX_VY, WORMHOLE_FLAP, WORMHOLE_GRAVITY, WORMHOLE_SPEED_BASE, WORMHOLE_SPEED_RAMP, WORMHOLE_WIDTH, WORMHOLE_TURN, WORMHOLE_DEBRIS_SPACING, WORM_EVERY_GATES, WORM_CALM_SECONDS, WORM_CALM_SPEED, WORM_EXIT_LEAD, WORM_EXIT_GRACE, } from "./control-constants.js?v=293";
...
export function resizeWorld(w, W, H) {
    ...
    const remapPlanets = w.flight !== "tunnel" && !w.tut && !w.race && !w.spill &&
...
export function pilotSuitId(w, save) {
    return w.tutSuit ? TUTORIAL_SUIT : save.equippedSuit;
}
export const FLIGHT_TEST_PATTERNS = ["manual", "hover", "2", "4", "6", "8", "pairs", "station"];
/** seconds between taps for the fixed-cadence patterns; hover is 2V/g */
export function flightTestInterval(pattern) {
    switch (pattern) {
...
export function beginFlightTest(w, save, pattern = "hover") {
    if (!IS_BETA)
        return false;
...
function stepFlightTest(w, save, dt) {
    const t = w.flightTest;
    while (t.tapLog.length && t.tapLog[0] < w.time - 6)
        t.tapLog.shift();
    if (t.pattern === "manual")
        return;
    t.next -= dt;
    ...
    if (t.pattern === "station") {
        ...
        if (w.ready || (w.squirrel.y > w.H * 0.5 && w.squirrel.vy > 0)) {
            flap(w, save);
            t.next = 0.1;
        }
        return;
...
export function resetRun(w, save, flight, tutorial, level, tunnelSeed) {
    ...
    w.spill = flight === "spill"
        ? createSpill(w.W, w.H, level ? level.seed : (Math.random() * 0x100000000) >>> 0, level ? level.spillFinish ? Number.MAX_SAFE_INTEGER : level.gates : 0, !save.helpOff)
        : null;
    w.spillCues = [];
    if (w.spill) {
...
export function spawnTrail(w, save, scale = 1) {
    ...
    if (STAR_CHART_TRAILS.has(trail))
        return;
...
export function flap(w, save) {
    ...
    if (w.flightTest) {
        ...
        if (w.flightTest.tapLog.length > 64)
            w.flightTest.tapLog.shift();
    }
...
function bounceOff(w, save, px, py, mul = 1) {
        ...
        w.lvl.stats.bounces += 1;
    {
        ...
        if (stick > 0 && (w.missionRng ?? Math.random)() < stick) {
            w.stuck = true;
            w.squirrel.vy = 0;
        }
    }
...
function die(w, save) {
    ...
    if (!w.lvl && !w.tut && w.score > (priorBest ?? 0))
        platform.celebrate();
...
function updateSpill(w, save, dt) {
    const s = w.spill;
...
export function updateWorld(w, save, dt) {
    ...
    if (w.race) {
        ...
        if (isHighOrbit(orbitRaceSuit) && w.race.tick > priorRaceTick && !w.tut?.hold
            ...
            if (isPremiumSuit(orbitRaceSuit))
                stepPremiumWake(w.highOrbit, orbitRaceSuit, RACE_DT, w.race.vy);
            else
    ...
    // retrofit study, 12 Sep 2026). Stepped always, drawn only with the dial.
    {
        ...
        if (w.tapReact > 1) {
            w.tapReact = 1;
            w.tapReactV = Math.min(0, w.tapReactV);
        }
    }
    ...
    if (isHighOrbit(orbitSuit) && !w.tut?.hold && !w.spill && w.shieldFreeze <= 0 && w.warpT <= 0 && !w.stuck) {
        ...
        if (isPremiumSuit(orbitSuit))
            stepPremiumWake(w.highOrbit, orbitSuit, visualDt, w.squirrel.vy, w.ready);
        else
    ...
    if (w.flightTest && w.screen === "play" && !w.tut)
        stepFlightTest(w, save, dt);
