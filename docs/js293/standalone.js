import { selectShopCycle } from "./shop-cycle.js?v=293";
import { spillControlArt, SPILL_CONTROL_LAYOUT } from "./spill-control-art.js?v=293";
import { suitPitchFor, tapShapeFor, tailSpringFor, tapAccentStrengthFor } from "./save.js?v=293";
import { platform } from "./platform.js?v=293";
import { trailWornBy, canWearTrail, builtInTrailSuit } from "./catalog.js?v=293";
import { isHighOrbit, HIGH_ORBIT_PROFILES } from "./high-orbit-config.js?v=293";
import { PLANNED_STAR_REWARDS } from "./star-map-rewards.js?v=293";
import { addChartScenery } from "./star-map-view.js?v=293";
import { mapDebrisIndex } from "./zone-visuals.js?v=293";
import { missionCredit, verifiedMask, routeMasks, rewardId } from "./campaign-progress.js?v=293";
import { STAR_MAP_PREVIEW, suitPitchDefault } from "./catalog.js?v=293";
import { repeatTapMode, FLIGHT_TEST_PATTERNS } from "./sim.js?v=293";
import { suitLean, TAP_SHAPE_MIN, TAP_SHAPE_MAX, TAIL_SPRING_MIN, TAIL_SPRING_MAX, TAIL_SPRING_SUITS, TAP_ACCENT_STRENGTH, TAP_ACCENT_MIN, TAP_ACCENT_MAX } from "./control-constants.js?v=293";
import { CHART_LEVELS, CHART_MAX_STARS, nextLevel, levelAt, reachedGate, SUB_ACORNS } from "./campaign.js?v=293";
import { ART_VER, BUILD, ENVS, GUIDE_HELM, GUIDE_SUIT, HELMETS, HELMET_SHELF, SUIT_SHELF, IAP_ITEMS, IS_BETA, MOD_SHIELD_COST, MODS, PALS, PHYS, SUITS, TRAILS, helmetWornBy, isIap, wearsOwnHead, BUNDLES, bundleIds, idDust, SET_TRAIL, fixedHeadTag, fixedHeadLine, fixedHeadDescription, DUST_PACKS, CASH_PACKS, DAILY_DUST, DAILY_STREAK_BONUS, DAILY_STREAK_LEN, BOOSTS, BOOST_IDS, DEV_STAMP, IAP_LIVE, ACORNS_PER_DUST } from "./catalog.js?v=293";
import { paintPortrait, paintTrailPreview, paintPalPreview, paintFlightPreview, paintShipPreview } from "./draw.js?v=293";
import { bundleQuote } from "./catalog.js?v=293";
import { drawSprite as drawSpriteOn } from "./art.js?v=293";
import { createEngine } from "./engine.js?v=293";
import { dualPalUnlocked, equippedPals, deepUnlocked, helmetRevealed, lostUnlocked, palUnlocked, startShieldUnlocked, suitRevealed, starsOf, trailUnlocked, PILOT_NAME_MAX, boostReady, skipEligible, rewardOwned, ownsPremium } from "./save.js?v=293";
import { HYPER_RUN_MAX_ACORNS, HYPER_RUN_MISSION, STAR_REWARDS, STAR_UNLOCKS, countBits, fxText, goalText, levelUnlocked, starTitle, RACE_GATES } from "./campaign.js?v=293";
import { formatRaceTicks } from "./race.js?v=293";
import { SPILL_UTILITIES, spillMastery, spillEngineColor } from "./spill-content.js?v=293";
import { spillPreviewState } from "./spill-presentation.js?v=293";
import { createDepotView, drawDepotWorkshop, drawSpillLaunchSetup, drawSpillEnginePicker, drawSpillFlightHelp, drawSpillGuideSheet } from "./spill-workshop.js?v=293";
...
export async function bootStandalone(root) {
    ...
    let keptShopRails = new Map();
    const keepShopRails = () => {
        keptShopRails = new Map([...overlay.querySelectorAll("[data-shop-rail]")]
            .map(r => [r.dataset.shopRail, { left: r.scrollLeft, top: r.scrollTop }]));
    };
    const restoreShopRails = () => {
        for (const rail of overlay.querySelectorAll("[data-shop-rail]")) {
            const kept = keptShopRails.get(rail.dataset.shopRail);
            if (kept) {
                rail.scrollLeft = kept.left;
                rail.scrollTop = kept.top;
            }
        }
    };
    const keepShelves = () => {
            ...
            if (snap.flight === "tunnel") {
                ...
                if (engine.save.guide !== "reward") {
                    ...
                    if (offer) {
                        ...
                        ad.onclick = () => {
                            void engine.continueWithAd().then((out) => {
                                if (out === "dismissed")
                                    announce("The ad was closed early — nothing earned.");
                                else if (out === "unavailable")
                                    announce("No ad is ready right now.");
                            });
                        };
                        sheet.append(ad);
                    }
    ...
    function headAside(acorns, paintedHelp = false) {
    ...
    function settingsRows() {
        ...
        row("Character glow", "The flash and recoil on every tap", () => !sv().glowOff, () => engine.setGlowOff(!sv().glowOff));
    ...
    function dialRow(label, min, max, step, value, fmt, commit) {
        const row = el("div", "ac-labrow");
        const name = el("span", "ac-labname", label);
    ...
    function tapShapeDial(suitId) {
        if (suitId === "vanguard" || suitId === "arcflash")
            return null; // their own controllers
    ...
    function tapAccentDial(suitId) {
        ...
        for (const [label, v] of [["OFF", false], ["ON", true]]) {
        ...
        if (on) {
            ...
            quick.style.gridTemplateColumns = "repeat(6, minmax(0,1fr))";
            for (const v of [0, 1, 2, 3, 4]) {
                const b = el("button", k === v ? "ac-mode on" : "ac-mode", `${v}×`);
                b.onclick = () => engine.setTapAccentStrength(suitId, v);
                quick.append(b);
            }
            const reset = el("button", "ac-mode", "STOCK");
            reset.onclick = () => engine.setTapAccentStrength(suitId, null);
            quick.append(reset);
            panel.append(quick);
        }
        return panel;
    }
    function tailSpringDial(suitId) {
        if (!TAIL_SPRING_SUITS.includes(suitId))
            return null; // the tail is painted into the frames
        const panel = el("div", "ac-suit-pitch");
        const name = (SUITS.find((s) => s.id === suitId)?.name ?? suitId).toUpperCase();
        ...
        const x = (v) => `${v.toFixed(2)}×`;
        const set = (patch) => engine.setTailSpring(suitId, { ...cur, ...patch });
        ...
        const row = el("div", "ac-modes");
        row.style.gridTemplateColumns = "repeat(1, minmax(0,1fr))";
        const reset = el("button", "ac-mode", "STOCK");
        reset.onclick = () => engine.setTailSpring(suitId, null);
        row.append(reset);
        panel.append(row);
        return panel;
    }
    function repeatTapDial(suitId) {
        if (suitId === "vanguard" || suitId === "arcflash")
            return el("i"); // their own controllers
        const panel = el("div", "ac-suit-pitch");
        const name = (SUITS.find((s) => s.id === suitId)?.name ?? suitId).toUpperCase();
        ...
        const row = el("div", "ac-modes");
        row.style.gridTemplateColumns = "repeat(4, minmax(0,1fr))";
        for (const [label, m] of [["REWIND", "rewind"], ["FINISH", "finish"], ["RESTART", "restart"]]) {
            const b = el("button", mode === m ? "ac-mode on" : "ac-mode", label);
            b.onclick = () => engine.setTapRepeat(suitId, m);
            row.append(b);
        }
        const stock = el("button", "ac-mode", "STOCK");
        stock.onclick = () => engine.setTapRepeat(suitId, null);
        row.append(stock);
        panel.append(row);
        return panel;
    }
    ...
    function drawHome() {
        ...
        tiles.append(launch);
        const loadoutTile = tile("t-loadout", hubIcon("star-chart-holo"), "LOADOUT", "Suits & gear", () => engine.open("hangar"), undefined, s.guide === "hangar" || s.guide === "helmet");
        ...
        const chart = tile("t-chart", hubIcon("star-chart-rocket"), "STAR CHART", nxt ? `Next unlock ★ ${nxt.stars}` : "Complete", () => engine.open("log"), undefined, s.guide === "levels");
        ...
        tile("t-modes", hubIcon("modes-orbit"), "MODES", `${MODES.length} ways to fly`, () => { modesOpen = true; render(); });
        box.append(tiles);
    ...
    function drawModeSheet() {
        ...
        const back = modeHead.querySelector(".ac-backbtn");
    ...
    function wornSuitDials(suitId) {
        ...
        if (spring)
            ...
            col.append(el("p", "ac-sub ac-testlab-note", "TAIL SPRING · this suit's tail is painted into its frames; nothing for the spring to move"));
    ...
    function drawTestLabSheet() {
        ...
        const back = head.querySelector(".ac-backbtn");
        ...
        sheet.append(labDoor("FLIGHT TEST", `${wornName} flies itself · hover, 2/4/6/8 taps a second, pairs, station · every dial in the dock`, () => { testLabOpen = false; resetArmed = false; engine.startFlightTest(); }, "ac-testlab-go"));
        sheet.append(el("p", "ac-modeshead", "BENCHES · SEPARATE PAGES"));
        for (const [label, sub, path] of LAB_PAGES)
            sheet.append(labDoor(label, sub, () => { window.location.href = labRootOf() + path; }));
        // two taps to reset, because it clears numbers the owner may not have
        // baked into the tables yet
        const reset = el("button", resetArmed ? "ac-ghost ac-testlab-reset on" : "ac-ghost ac-testlab-reset", resetArmed ? "TAP AGAIN TO RESET EVERY DIAL" : "RESET ALL DIALS TO STOCK");
        reset.onclick = () => { if (resetArmed) {
            resetArmed = false;
            engine.resetTestLab();
        }
        else {
    ...
    function flightTestDock() {
        ...
        const read = (label) => { const d = el("div"); const b = el("b", "", "—"); d.append(el("span", "", label), b); reads.append(d); return b; };
        ...
        const seg = (items) => {
            ...
            for (const [label, on, hit] of items) {
                const b = el("button", on ? "ac-mode on" : "ac-mode", label);
                b.onclick = hit;
                row.append(b);
            }
        ...
        const patLabel = { manual: "MANUAL", hover: "HOVER", "2": "2/s", "4": "4/s", "6": "6/s", "8": "8/s", pairs: "PAIRS", station: "STATION" };
        dock.append(seg(FLIGHT_TEST_PATTERNS.map((p) => [patLabel[p], ft.pattern === p, () => engine.setFlightTest({ pattern: p })])));
        ...
        dock.append(seg([
            ...
            ["STEP", false, () => { if (!held)
                    engine.setFlightTest({ speed: 0 }); engine.flightTestStep(); }],
        ]));
        const worn = engine.save.equippedSuit;
        ...
        const tick = () => {
            ...
            const pose = window.__acornautPose;
    ...
    function palCardOf(pl, forShop = false) {
        ...
        if (starGate !== undefined)
            b.append(unlockTag(starGate));
    ...
    function drawHangar() {
        ...
        if (engine.shopTab === "helmets") {
            ...
            if (isHighOrbit(s.equippedSuit))
                grid.append(el("p", "ac-sub", `${HIGH_ORBIT_PROFILES[s.equippedSuit].wake} belongs to ${HIGH_ORBIT_PROFILES[s.equippedSuit].name}. Your previous trail returns when you change suits.`));
    ...
    function drawShopBeta() {
        ...
        if (ownHead)
            plate.append(el("span", "ac-tonohelm ac-casetag", fixedHeadTag(suit)));
        ...
        if (ctx) {
            ...
            const tick = () => {
                ...
                ctx.clearRect(0, 0, CASE_W, CASE_H);
                if (palDef)
                    paintPalPreview(ctx, engine.art, palDef.id, CASE_W - 30, 43, 38);
                const low = lowSeatPal(engine.save, palDef?.id);
                if (low)
                    paintPalPreview(ctx, engine.art, low, CASE_W - 30, 137, 38);
        ...
        if (!cart.ids.length) {
            ...
            t.append(el("b", "", "NOTHING SELECTED"), el("span", "", "Tap a card to add it."));
            bar.append(t);
        }
        else {
            const t = el("span", "ac-combotxt");
            ...
            clear.onclick = () => {
                picked = new Set();
                render();
                overlay.querySelector('[data-focus="shop:cart"]')?.focus({ preventScroll: true });
            };
            ...
            go.onclick = () => {
                ...
                if (bought) {
                    render();
                    (overlay.querySelector('[data-focus="shop:buy"]')
                        ?? overlay.querySelector('[data-focus="shop:cart"]'))?.focus({ preventScroll: true });
                    if (refused) {
                        announce(refused);
        ...
        const shelf = (kind, ids) => {
            ...
            for (const id of ids) {
                ...
                b.onclick = () => {
                    ...
                    render();
                    overlay.querySelector(`[data-focus="shop:${kind}:${id}"]`)?.focus({ preventScroll: true });
                };
                row.append(b);
            }
        };
        ...
        if (platform.adsReady) {
            ...
            row.append(face);
            const t = el("div", "ac-modtxt");
            t.append(el("p", "ac-modname", `WATCH AN AD · +${ad.pays} STAR DUST`), el("p", "ac-sub", ad.busy ? "Ad playing…" : ad.left > 0 ? `${ad.left} left today` : "Back tomorrow"));
            ...
            row.onclick = () => {
                void engine.watchAdForDust().then((out) => {
                    if (out === "dismissed")
                        announce("The ad was closed early — nothing earned.");
                    else if (out === "unavailable")
                        announce("No ad is ready right now.");
                    else if (out === "spent")
                        announce("That is all for today — back tomorrow.");
                    else
                        clearDeny();
                });
            };
        ...
        for (const dp of DUST_PACKS) {
            ...
            row.append(face);
            const t = el("div", "ac-modtxt");
            t.append(el("p", "ac-modname", `${(dp.dust + dp.bonus).toLocaleString()} Star Dust`), el("p", "ac-sub", dp.bonus ? `${dp.dust.toLocaleString()} + ${dp.bonus} bonus`
            ...
            if (dp.cash && !platform.storeReady && !IS_BETA && !IAP_LIVE)
                continue;
            if (!dp.cash) {
                ...
                if (inFlight)
                    row.disabled = true;
                row.onclick = () => { if (inFlight)
                    return; tx(row, () => engine.buyDust(dp.id), dp.acorns, "acorns"); render(); };
                scroll.append(row);
                continue;
            }
    ...
    function drawCycleRoll(cy) {
        ...
        for (const id of IAP_ITEMS) {
            if (cy.owns(id))
                owned.push(id);
            else if (cy.excluded.has(id))
                loginRewards.push(id);
        ...
        const group = (title, ids, why, showPrice = true) => {
    ...
    function drawHelp() {
        ...
        const scroll = el("div", "ac-sheet-scroll");
        const modeSection = (id, title, theme) => {
        ...
        const control = (glyph, title, action, keys, direction) => {
            const card = el("div", `ac-helpcontrol ac-help-${direction}`);
            const mark = el("span", "ac-helpglyph", glyph);
        ...
        controls.append(control("\u25B2", "TAP", "BOOST UP", "Space / \u2191", "rise"), control("\u25BC", "SWIPE DOWN", "DIVE", "\u2193 / S / Ctrl", "dive"));
        ...
        const spillControls = spillHelp.querySelector(".ac-spillhelp-controls");
        if (spillControls)
            debris.append(spillControls);
        const extras = el("div", "ac-helpextras");
        const harderKick = spillHelp.querySelector(".ac-spillhelp-extra");
        if (harderKick)
            extras.append(harderKick);
        ...
        debris.append(extras);
        const hyper = modeSection("hyper", "Hyper Run Mode", "m-race");
        const hyperControls = el("div", "ac-helpcontrols");
        hyperControls.append(control("\u25B2", "HOLD TO RISE", "RELEASE TO FALL", "Hold Space / \u2191", "rise"), control("\u25BC", "DIVE", "SWIPE DOWN", "\u2193 / S / Ctrl", "dive"));
        const hyperExtras = el("div", "ac-helpextras");
        hyperExtras.append(el("small", "ac-helpkeys", "Double-tap + hold: boost"), el("small", "ac-helpkeys", "Tunnel: drag to steer"));
