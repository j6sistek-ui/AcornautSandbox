export function prepareDepotBear(sheet) {
    ...
    for (let row = 0; row < 6; row++)
        for (let col = 0; col < 6; col++) {
            ...
            image.width = Math.ceil(width / 6);
            image.height = Math.ceil(height / 6);
            ...
            const visit = (p) => {
                ...
                if (!data[i + 3] || (low >= 236 && high - low <= 24))
                    queue[write++] = p;
            };
            for (let x = 0; x < W; x++) {
                visit(x);
            ...
            while (read < write) {
                const p = queue[read++], x = p % W, y = Math.floor(p / W);
                data[p * 4 + 3] = 0;
                if (x)
                    visit(p - 1);
                if (x + 1 < W)
            ...
            for (let start = 0; start < W * H; start++) {
                if (islands[start] || !data[start * 4 + 3])
                    continue;
                ...
                const add = (p) => {
                    ...
                    if (data[i + 3] && low >= 249 && Math.max(data[i], data[i + 1], data[i + 2]) - low <= 8)
                        island[count++] = p;
                };
                add(start);
                while (cursor < count) {
                ...
                for (let x = 1; x < W - 1; x++) {
                    ...
                    for (const n of [p - W, p + W, p - 1, p + 1, p - W - 1, p - W + 1, p + W - 1, p + W + 1]) {
                        ...
                        if (!original[j + 3])
                            ...
                            inner = Math.min(inner, original[j], original[j + 1], original[j + 2]);
                    }
                    if (!edge || low - inner < 10)
                        continue;
                    const alpha = Math.max(.15, Math.min(1, (255 - low) / Math.max(1, 255 - inner)));
                    for (let c = 0; c < 3; c++)
                        data[i + c] = Math.max(0, (original[i + c] - 255 * (1 - alpha)) / alpha);
                    data[i + 3] = Math.round(255 * alpha);
                }
            let left = W, right = 0, bottom = 0;
            for (let y = Math.floor(H * .65); y < H; y++)
                for (let x = 0; x < W; x++) {
