import { canWearTrail, builtInTrailSuit, STAR_MAP_PREVIEW, ENV_GATES, palsClash, IAP_LIVE, AD_RULES } from "./catalog.js?v=291";
import { platform } from "./platform.js?v=291";
import { beginFlightTest } from "./sim.js?v=291";
import { TAP_SHAPE_MIN, TAP_SHAPE_MAX, TAIL_SPRING_MIN, TAIL_SPRING_MAX, TAP_ACCENT_STRENGTH, TAP_ACCENT_MIN, TAP_ACCENT_MAX } from "./control-constants.js?v=291";
import { isPremiumSuit } from "./high-orbit-config.js?v=291";
import { spillAppearance } from "./spill-appearance.js?v=291";
import { routeMasks, rewardId } from "./campaign-progress.js?v=291";
import { reachedGate } from "./campaign.js?v=291";
import { emptyArt, loadArt, loadPalBank, loadSuitBank, loadSpillScene, loadZoneArt, prefetchArtBanks } from "./art.js?v=291";
import { vanguardDepotEligible } from "./spill-depot-gag.js?v=291";
import { sfx, unlockAudio, music, setSfxMuted } from "./audio.js?v=291";
import { GUIDE_HELM, GUIDE_SUIT, TUTORIAL_SUIT, HELMETS, IAP_ITEMS, PALS, HYPER_RUN_ENABLED, IS_BETA, isIap, MOD_BATTERY_COST, MOD_SHIELD_COST, MODS, SUITS, TRAILS, TUT_ARM, BUNDLES, bundleIds, bundlePrice, idDust, idGrants, featurePrice, DUST_PACKS, DAILY_DUST, DAILY_STREAK_BONUS, DAILY_STREAK_LEN } from "./catalog.js?v=291";
import { drawHud, drawWorld, setSpillBackplateHost } from "./draw.js?v=291";
import { setVanguardPitchTrim } from "./vanguard.js?v=291";
import { batteryUnlocked, deepUnlocked, helmetRevealed, trailUnlocked, eraseSave, lostUnlocked, modsUnlocked, loadSave, grantTutorialKit, palUnlocked, startShieldUnlocked, starsOf, suitRevealed, writeSave, cleanPilotName, dualPalUnlocked, } from "./save.js?v=291";
import { hyperRunById, levelById, levelUnlocked, STAR_REWARDS } from "./campaign.js?v=291";
import { dive, envIndexFor, flap, initStars, makeWorld, pausePlay, planRaceCueEffects, resizeWorld, resetRun, resumePlay, reviveCost, reviveRun, canRevive, setRaceInput, snapshot, takeRaceCueEffects, takeSpillCues, spillBurstUp, updateWorld, } from "./sim.js?v=291";
import { canonicalRaceY, cancelRaceGesture, createRaceGestureState, dropRaceGesture, moveRaceDragGesture, moveRaceGesture, neutralizeOwnedRaceGesture, pressRaceDragGesture, pressRaceGesture, pressRaceKeyboardDragGesture, releaseRaceGesture, } from "./race-gesture.js?v=291";
import { raceViewport } from "./race-viewport.js?v=291";
import { spillBuy, spillLeaveDepot, spillLunge, spillUtility, spillSpecialize, spillTakeContract, spillCheckpoint, restoreSpill } from "./spill.js?v=291";
import { SPILL_UTILITIES, SPILL_ENGINE_COLORS, spillEngineColor } from "./spill-content.js?v=291";
import { bankSpill, suitPitchFor, takeReceipt, buyBoost, skipLevel, unlockReward, ownsPremium, settleStarRewards } from "./save.js?v=291";
export async function createEngine(canvas) {
    ...
    const muteAll = (m) => { shellMuted = m; applyMute(); };
    ...
    const engine = {
        ...
        setTapAccentStrength(suitId, k) {
            if (!save.tapAccentStrength)
                save.tapAccentStrength = {};
            if (k === null)
                delete save.tapAccentStrength[suitId];
            else {
                const v = Math.max(TAP_ACCENT_MIN, Math.min(TAP_ACCENT_MAX, Math.round(k * 4) / 4));
                // the table's own value is not an override
                if (v === (TAP_ACCENT_STRENGTH[suitId] ?? 1))
                    delete save.tapAccentStrength[suitId];
                else
                    save.tapAccentStrength[suitId] = v;
            }
            writeSave(save);
            notify();
        },
...
export { deepUnlocked, lostUnlocked } from "./save.js?v=291";
