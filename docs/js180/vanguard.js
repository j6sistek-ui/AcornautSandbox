 ...
 */
export function vanguardFrame(tap, bounce, vy) {
    if (bounce >= 0 && bounce < 0.38)
        return 24 + Math.min(7, Math.floor(bounce / 0.38 * 8));
    if (vy > 80)
        return VANGUARD_DIVE_ORDER[Math.min(7, Math.floor((vy - 80) / 540 * 8))];
    if (tap >= 0 && tap < VANGUARD_TAP_SECONDS)
        return VANGUARD_TAP_ORDER[Math.min(15, Math.floor(tap / VANGUARD_TAP_SECONDS * 16))];
    return 0;
}
 ...
 */
export function paintVanguard(ctx, art, x, y, size, tap, bounce, vy) {
