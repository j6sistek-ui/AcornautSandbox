import { spillDockTravelDuration } from "./spill.js?v=290";
import { clipHelmetGlass } from "./helmet-openings.js?v=290";
import { paintVanguardDepot, vanguardDepotPose } from "./spill-depot-gag.js?v=290";
import { paintVanguard, paintVanguardShield, paintVanguardWake, paintVanguardContacts, vanguardPreview } from "./vanguard.js?v=290";
import { paintArcflash, paintArcflashWake, paintArcflashCockpit } from "./arcflash.js?v=290";
import { arcflashPreview } from "./arcflash-motion.js?v=290";
import { paintLiveStarTrail } from "./star-trails.js?v=290";
import { trailWornBy } from "./catalog.js?v=290";
import { isHighOrbitRig, isPremiumSuit, highOrbitTrailSuit } from "./high-orbit-config.js?v=290";
import { paintHighOrbit, paintHighOrbitCockpit } from "./high-orbit.js?v=290";
import { highOrbitPreview, premiumWakePreview } from "./high-orbit-motion.js?v=290";
import { paintPremiumBankWake } from "./premium-bank-wake.js?v=290";
import { CYBER_TRIO_REGISTRATION } from "./cyber-trio-registration.js?v=290";
import { paintHighOrbitWake } from "./high-orbit-effects.js?v=290";
import { runPals, fxOf, worldFlipped } from "./sim.js?v=290";
import { spillAppearance } from "./spill-appearance.js?v=290";
import { hasZoneRemaster, zonePainting, zoneVisual } from "./zone-visuals.js?v=290";
import { samplePlanetBackdrop } from "./planet-contrast.js?v=290";
import { SKY_RGB, PLANET_RGB, BOUNCE_ANIM_DURATION, ENVS, PHYS, SUITS, TAIL, TAP_ANIM_DURATION, helmetWornBy, skyIdFor, washScale, wearsOwnHead } from "./catalog.js?v=290";
import { goalHud } from "./campaign.js?v=290";
import { drawTrailPreviewOn, drawPalOn, drawAstronautOn, canDrawPal } from "./cosmetics.js?v=290";
import { proceduralSky, hueShifted } from "./sky-gen.js?v=290";
import { drawSprite, skyImage, spriteHalo, SPRITE_HALO_PAD } from "./art.js?v=290";
import { spriteImageFor } from "./sprite-detail.js?v=290";
import { retroBackdrop, retroPlanet, retroObstacle, retroAcorn, retroBlocker } from "./retro.js?v=290";
import { suitPitchFor, tapShapeFor, tapAccentStrengthFor } from "./save.js?v=290";
import { blockerX, gateOffset, liveGapY, pilotSuitId, tiltNow, tunnelBoundsAt, WORM_TRIP_SECONDS } from "./sim.js?v=290";
import { WORM_EXIT_LEAD, suitLean, SUIT_LEAN_DEFAULT, PAINTED_TAP_EASE } from "./control-constants.js?v=290";
import { raceViewport, raceViewportX, raceViewportY } from "./race-viewport.js?v=290";
import { SPILL, SPILL_MOD_INFO, spillHas, spillChargeCap, spillContractProgress, spillEventGap, spillCount, spillMod, spillRamp, spillWaveLeft, } from "./spill.js?v=290";
import { spillEngineColor } from "./spill-content.js?v=290";
import { SPILL_MODULE_MARKS, spillDockBear, spillDockView, spillPreviewState } from "./spill-presentation.js?v=290";
import { RACE_ACORNS, RACE_BASE_SPEED, RACE_DEBRIS, RACE_ENTRY_TICKS, RACE_GATE_CLEARANCE, RACE_GATE_MISS_FADE_TICKS, RACE_GATE_PASS_FADE_TICKS, RACE_HZ, RACE_LENGTH, RACE_MAX_INTERACTIVE_GAP, RACE_MAX_SPEED, RACE_PILOT_X, RACE_READY_COPY, RACE_RETURN_TICKS, RACE_RINGS, RACE_TUNNEL_PERFECT_APERTURE, RACE_TUNNEL_RING_APERTURE, RACE_TUNNEL_SPEED, RACE_TUNNEL_TICKS, formatRaceTicks, raceDecisionAge, raceRouteTarget, raceTunnelGeometry, raceTunnelQuality, raceTunnelRings, } from "./race.js?v=290";
...
function drawSpillRock(ctx, art, r) {
    const img = art.debris[r.sprite % Math.max(1, art.debris.length)];
    ctx.save();
    ctx.translate(r.x, r.y);
    ctx.rotate(r.rot);
    if (img)
        drawSprite(ctx, img, 0, 0, r.r * 2, "core", "light");
    else {
...
function drawSpillWarning(ctx, w, r) {
...
function drawSpillShip(ctx, w, save, art, s, x, parked = false) {
...
function drawSpillWorld(ctx, w, save, art) {
    ...
    if (dock && parked) {
        ...
        if (cameo) {
            ...
            paintVanguardDepot(ctx, art.spillScene.vanguardDepot, actorX, actorY, marshal.height * .9, gagTime);
            if (pose.enter > 0) {
                ...
                ctx.fillStyle = glow;
                ctx.fillRect(px - marshal.height, py - marshal.height, marshal.height * 2, marshal.height * 2);
            }
        }
        if (bear) {
        ...
        if (cameo) {
            ...
            drawSpillShip(ctx, w, save, art, s, s.pilot.x, true);
            ctx.restore();
            ctx.restore();
            return;
        }
...
function drawSpillHint(ctx, w, text, alpha, bottom) {
    ...
    if (ctx.measureText(body).width > bw - 28) {
        ...
        for (let i = body.indexOf(sep); i >= 0; i = body.indexOf(sep, i + 1)) {
            if (best < 0 || Math.abs(i - body.length / 2) < Math.abs(best - body.length / 2))
                best = i;
        }
        if (best > 0) {
            lines.push(body.slice(0, best), body.slice(best + sep.length));
            body = "";
        }
    }
    if (body)
...
function paintDome(ctx, body, key, helmet, x, y, size, art) {
    ...
    const box = naturalFlightKey(key) ? NATURAL_FLIGHT_BOX
        : body.box ?? { x: 0, y: 0, w: body.width, h: body.height };
...
export const FROZEN_SUITS = ["vanguard", "arcflash", "robo", "bigbooty", "catsuit", "volt"];
...
let bankPose = 0;
let bankPoseClock = -1;
function easeBankPose(t, target, exact) {
    ...
    if (exact || fresh)
        ...
        bankPose += (target - bankPose) * (1 - Math.exp(-dt / DIVE_ENTRY_SECONDS));
    return bankPose;
}
    ...
    if (suited && ((rigT && rigB) || bankReady)) {
        ...
        if (fullBounce) {
            ...
            if (!preShaped && !Number.isFinite(poseOverride)) {
                ...
                if (ascFrames.length > 1 && !velocityAscent)
            ...
            const refM = NATURAL_FLIGHT_SUITS.has(suit.id) ? NATURAL_FLIGHT_BOX : ascFrames[0].box ?? ref;
            if (isPremiumSuit(suit.id) && wake)
                paintPremiumBankWake(ctx, suit.id, diving ? "desc" : "asc", idxM, refM, x, y, size, wake);
            ...
            const idx = order[Math.min(order.length - 1, Math.floor((tapAnimT / TAP_ANIM_DURATION) * order.length))];
...
function drawPilot(ctx, w, save, art, xOverride, localScale = 1, yOverride, bankScale = 0.8, effects = true) {
    ...
    if (accentK > 0) {
        ...
        if (w.flapBoost > 0 && age < 0.167) {
            ...
            const col = suit.glow ?? helm.glow ?? "#4ad8ff";
    ...
    if (!independentRig && !eclipseImpact && sq > 0)
    ...
    if (flagship)
    ...
    else if (isHighOrbitRig(suit.id))
        ...
        w.bounceAnimT, w.bounceAnimDir, w.bounceAnimStrength, w.squirrel.vy, ECLIPSE_FLIGHT_SUITS.has(suit.id) ? 2 : 0, w.speed, lean, NaN, true, tapShapeFor(save, suit.id), effects && isPremiumSuit(suit.id) ? { state: w.highOrbit, travel: w.distance / localScale } : undefined);
...
export function paintPortrait(ctx, art, helmet, suit, cx, cy, size, _t = 0) {
    ...
    if (!body)
        return;
    const key = art?.suits?.[suit.id] ? "suit:" + suit.id : "idle-1";
    if (naturalFlightKey(key))
        drawRigLayer(ctx, body, NATURAL_FLIGHT_BOX, cx, cy + 2, size);
    else
...
pitch = 0) {
    ...
    ctx.scale(pop, pop);
    paintIllustrated(ctx, frames?.[idx] ?? null, 0, 2, 52, helmet, suit, t, art, (flapping ? "flap-" : "idle-") + (idx + 1), frames?.[nxt] ?? null, (flapping ? "flap-" : "idle-") + (nxt + 1), blend, "light", previewTailAngle(p, BEAT), tapAnimT, -1, 0, 0, vy, 2, 300, lean, sweptPose, false, tapShapeFor(null, suit.id), // the table only: a preview has no save
