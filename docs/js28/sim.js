import { PLANET_COUNT, PLANET_LUM, SKY_LUM, LUM_GAP, ENVS, ENV_GATES, PHYS, TRAILS, TUT_ARM, levelForXp, runXp } from "./catalog.js?v=28";
import { writeSave } from "./save.js?v=28";
...
function pickKind(w) {
    ...
    const skyL = SKY_LUM[env.sky];
    for (let i = 0; i < 10; i++) {
        const k = Math.floor(Math.random() * PLANET_COUNT);
        if (Math.abs((PLANET_LUM[k] ?? 0.4) - skyL) >= LUM_GAP)
            return k;
    }
    return env.planetBias[Math.floor(Math.random() * env.planetBias.length)] % PLANET_COUNT;
}
