...
const ID: Xf = { dx: 0, dy: 0, scale: 1, rot: 0 };
...
const artRoot = () => ((window as any).__ACORNAUT_ART__ as string) || "../../art";

async function loadSprite(name: string): Promise<Sprite> {
  const img = new Image();
  ...
  if (x1 < x0) { x0 = y0 = 0; x1 = c.width - 1; y1 = c.height - 1; }
  return { img, cx: (x0 + x1) / 2, cy: (y0 + y1) / 2, box: [x0, y0, x1, y1] };
}

export async function bootShip(root: HTMLElement) {
  const loading = el("div", "rg-boot", "loading the ship…");
  root.append(loading);

  const man: Manifest = await (await fetch(`${artRoot()}/spill-ship/manifest.json?v=${Date.now()}`)).json();
  ...
  const S = {
    hull: man.hulls[0],
    level: { } as Record<string, number>,      // per axis: 0 = off, 1..3
    sel: man.order[0],                          // the axis being edited
    xf: {} as Record<string, Xf>,               // part name -> transform
    over: {} as Record<string, Record<string, Xf>>, // hull -> part -> transform
    onlyHull: false,                            // edits go to the override for this hull
    step: 1,
    zoom: 1,
    showBox: true,
  };
  for (const k of man.order) S.level[k] = 1;

  const restore = () => {
    try {
      const d = JSON.parse(localStorage.getItem(STORE) || "null");
      if (d) { S.xf = d.xf || {}; S.over = d.over || {}; }
    } catch { /* a corrupt draft is not worth a broken page */ }
  };
  const save = () => { try { localStorage.setItem(STORE, JSON.stringify({ xf: S.xf, over: S.over })); } catch { /* private mode */ } };
  restore();

  const partOf = (axis: string) => S.level[axis] ? man.parts[axis][S.level[axis] - 1] : null;
  const xfFor = (part: string, hull = S.hull): Xf => S.over[hull]?.[part] ?? S.xf[part] ?? ID;
  const editing = (): Xf => {
    const p = partOf(S.sel); if (!p) return { ...ID };
    if (S.onlyHull) { S.over[S.hull] ??= {}; S.over[S.hull][p] ??= { ...xfFor(p) }; return S.over[S.hull][p]; }
    S.xf[p] ??= { ...ID }; return S.xf[p];
  };
  ...
  const mkSel = (opts: { v: string; t: string }[], val: string, on: (v: string) => void) => {
    ...
    for (const o of opts) { const op = el("option", "", o.t); op.value = o.v; s.append(op); }
  ...
  const nudge = (dx: number, dy: number) => { const x = editing(); x.dx += dx * S.step; x.dy += dy * S.step; save(); paint(); };
  const padBtn = (t: string, f: () => void) => { const b = el("button", "rg-pb", t); b.onclick = f; return b; };
  const stepBtn = el("button", "rg-pb", "1");
  stepBtn.onclick = () => { S.step = S.step === 1 ? 5 : S.step === 5 ? 0.25 : 1; stepBtn.textContent = String(S.step); };
  pad.append(el("div", "rg-pc"), padBtn("▲", () => nudge(0, -1)), el("div", "rg-pc"),
    padBtn("◀", () => nudge(-1, 0)), stepBtn, padBtn("▶", () => nudge(1, 0)),
    el("div", "rg-pc"), padBtn("▼", () => nudge(0, 1)), el("div", "rg-pc"));
  const dials = el("div", "rg-dials");
  const dial = (label: string, minus: () => void, plus: () => void) => {
    const d = el("div", "rg-dial");
    const m = el("button", "rg-pb", "−"); m.onclick = minus;
    const p = el("button", "rg-pb", "+"); p.onclick = plus;
    d.append(m, el("div", "rg-dl", label), p); return d;
  };
  dials.append(
    dial("SIZE", () => { const x = editing(); x.scale = Math.max(0.2, +(x.scale / 1.02).toFixed(4)); save(); paint(); },
      () => { const x = editing(); x.scale = Math.min(4, +(x.scale * 1.02).toFixed(4)); save(); paint(); }),
    dial("TURN", () => { const x = editing(); x.rot = +(x.rot - (S.step >= 1 ? S.step : 0.25)).toFixed(2); save(); paint(); },
      () => { const x = editing(); x.rot = +(x.rot + (S.step >= 1 ? S.step : 0.25)).toFixed(2); save(); paint(); }),
  );
  const acts = el("div", "rg-acts");
  const resetBtn = el("button", "rg-act rg-danger", "RESET PART");
  resetBtn.onclick = () => {
    const p = partOf(S.sel); if (!p) return;
    if (S.onlyHull) delete S.over[S.hull]?.[p]; else delete S.xf[p];
    save(); paint(); flash("part back to zero");
  };
  ...
  let toastT = 0;
  const flash = (t: string) => { toast.textContent = t; toast.classList.add("on"); clearTimeout(toastT); toastT = window.setTimeout(() => toast.classList.remove("on"), 1400); };
  ...
  function drawShip(g: CanvasRenderingContext2D, size: number, hull: string, box: boolean) {
    ...
    for (const axis of man.order) {
      ...
      if (box && axis === S.sel) {
        g.strokeStyle = "rgba(120,200,255,.9)"; g.lineWidth = 1 / (z * x.scale); g.setLineDash([4 / (z * x.scale), 3 / (z * x.scale)]);
  ...
  function paint() {
    ...
    for (let i = 0; i <= 256; i += 32) { const q = (i / 256) * size; ctx.beginPath(); ctx.moveTo(q, 0); ctx.lineTo(q, size); ctx.moveTo(0, q); ctx.lineTo(size, q); ctx.stroke(); }
    drawShip(ctx, size, S.hull, S.showBox);
    if (flight) {
      // the ship at the size the Spill flies it, in the corner, on deep space
      const fs = 58, pad = 10;
      ctx.fillStyle = "#07091a"; ctx.fillRect(size - fs - pad * 2, size - fs - pad * 2, fs + pad * 2, fs + pad * 2);
      ctx.save(); ctx.translate(size - fs - pad, size - fs - pad); ctx.imageSmoothingQuality = "high";
  ...
  cv.addEventListener("pointerdown", (e) => {
    ...
    if (pts.size === 1) { const x = editing(); drag = { x: e.clientX, y: e.clientY, dx: x.dx, dy: x.dy }; }
    else if (pts.size === 2) {
      const [a, b] = [...pts.values()]; const x = editing();
      pinch = { d: Math.hypot(b.x - a.x, b.y - a.y), a: Math.atan2(b.y - a.y, b.x - a.x), scale: x.scale, rot: x.rot }; drag = null;
    }
  });
  ...
  const end = (e: PointerEvent) => { pts.delete(e.pointerId); if (pts.size < 2) pinch = null; if (pts.size === 0) { drag = null; save(); } };
  cv.addEventListener("pointerup", end); cv.addEventListener("pointercancel", end);
  cv.addEventListener("wheel", (e) => {
    e.preventDefault(); const x = editing();
    if (e.altKey) x.rot = +(x.rot + (e.deltaY > 0 ? 1 : -1) * (S.step >= 1 ? S.step : 0.25)).toFixed(2);
    else x.scale = Math.max(0.2, Math.min(4, +(x.scale * (e.deltaY > 0 ? 1 / 1.03 : 1.03)).toFixed(4)));
    save(); paint();
  }, { passive: false });
  window.addEventListener("keydown", (e) => {
    if ((e.target as HTMLElement)?.tagName === "TEXTAREA") return;
    const k = e.key;
    if (k === "ArrowLeft") nudge(-1, 0); else if (k === "ArrowRight") nudge(1, 0);
    else if (k === "ArrowUp") nudge(0, -1); else if (k === "ArrowDown") nudge(0, 1);
    else return;
    e.preventDefault();
  });
  ...
  function openSheet() {
    ...
    const pre = el("pre", "rg-pre", json); inner.append(pre);
    ...
    dl.onclick = () => {
      const b = new Blob([json], { type: "application/json" }); const a = document.createElement("a");
      a.href = URL.createObjectURL(b); a.download = "spill-ship-transforms.json"; a.click();
      setTimeout(() => URL.revokeObjectURL(a.href), 4000);
    };
    const paste = el("button", "rg-act", "PASTE");
    paste.onclick = () => {
      const raw = window.prompt("paste a transforms JSON"); if (!raw) return;
      try { const d = JSON.parse(raw); S.xf = d.parts || {}; S.over = d.overrides || {}; save(); sheet.remove(); paint(); flash("loaded"); }
      catch { flash("not valid JSON"); }
    };
    ...
    const close = el("button", "rg-act", "CLOSE"); close.onclick = () => sheet.remove();
    row.append(cp, dl, paste, clr, close); inner.append(row); sheet.append(inner); root.append(sheet);
  }
  async function copy(text: string) {
    try { await navigator.clipboard.writeText(text); }
    catch {
      const ta = el("textarea", "rg-ta"); ta.value = text; root.append(ta); ta.select();
      try { document.execCommand("copy"); } catch { /* leave it selected */ }
  ...
  syncChrome();
  paint();
  (window as any).__ship = { S, paint, report };
}
