...
type Nut = { x: number; y: number; vx: number; vy: number; got: boolean; bob: number };
...
function step(s: Spill, dt: number) {
  ...
  if (s.pilot.y > s.H - 22) { s.pilot.y = s.H - 22; s.pilot.vy = Math.min(0, s.pilot.vy); }
...
function draw(ctx: CanvasRenderingContext2D, s: Spill, bank: Bank) {
  ...
  for (const n of s.nuts) {
    ...
    if (bank.acorn) ctx.drawImage(bank.acorn, n.x - 14, y - 14, 28, 28);
    else {
  ...
  if (s.phase !== "over") drawPilot(ctx, bank, s);
