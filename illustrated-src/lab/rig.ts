...
type HelmRow = { id: string; name: string; file: string; glass: Glass };
...
function restoreLocal() {
  ...
  try {
    ...
    for (const s of S.tables.suits) if (d.suits?.[s.key]) s.dome = d.suits[s.key].slice(0, 3);
...
export async function bootRig(root: HTMLElement) {
  ...
  const helmSel = mkSel(
    tables.helmets.map((h) => ({ v: h.id, t: h.name })),
    S.helm,
    (v) => {
      S.helm = v;
      build();
  ...
  dials.append(
    mkDial("SIZE", () => withActive((s, h) => resize(1 / 1.02, s, h)), () => withActive((s, h) => resize(1.02, s, h))),
    mkDial("ROT", () => withActive((s, h) => spin(-2, s, h)), () => withActive((s, h) => spin(2, s, h))),
  );
  ...
  // ---- tiles
  type Tile = { cv: HTMLCanvasElement; s: SuitRow; h: HelmRow; size: number; wrap: HTMLElement };
  let tiles: Tile[] = [];
  ...
  function pairs(): [SuitRow, HelmRow][] {
    ...
    if (S.mode === "suit") return tables.helmets.map((h) => [suitOf(S.suit), h] as [SuitRow, HelmRow]);
    if (S.mode === "helm") return wearable.map((s) => [s, helmOf(S.helm)] as [SuitRow, HelmRow]);
    const out: [SuitRow, HelmRow][] = [];
    for (const s of wearable) for (const h of tables.helmets) out.push([s, h]);
    return out;
  }
