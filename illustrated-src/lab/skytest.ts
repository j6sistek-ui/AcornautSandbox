...
type Palette = { base: RGB; glow: RGB; hues: RGB[] };
...
function paletteFrom(img: HTMLImageElement): Palette {
  const c = document.createElement("canvas");
  c.width = 24; c.height = 24;
  const cc = c.getContext("2d")!;
  cc.drawImage(img, 0, 0, 24, 24);
  const d = cc.getImageData(0, 0, 24, 24).data;
  const px: { rgb: RGB; lum: number; sat: number; hue: number }[] = [];
  ...
  const avg = (list: typeof px): RGB => {
    const n = Math.max(1, list.length);
    return [0, 1, 2].map((k) => Math.round(list.reduce((s, p) => s + p.rgb[k], 0) / n)) as RGB;
  };
  ...
  const hues: RGB[] = [];
  const usedHue: number[] = [];
  for (const p of bySat) {
    if (usedHue.some((h) => Math.min(Math.abs(h - p.hue), 360 - Math.abs(h - p.hue)) < 42)) continue;
    hues.push(p.rgb); usedHue.push(p.hue);
    if (hues.length === 3) break;
  }
  while (hues.length < 3) hues.push(glow);
...
function renderProceduralSky(w: number, h: number, envIndex: number, pal: Palette): HTMLCanvasElement {
  ...
  const punch = (rgb: RGB): RGB => {
    const m = (rgb[0] + rgb[1] + rgb[2]) / 3;
    return [0, 1, 2].map((k) => Math.min(255, m + (rgb[k] - m) * 2.1 + 26)) as RGB;
  };
  ...
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      ...
      for (const L of layers) {
        ...
        r += L.rgb[0] * val; gg += L.rgb[1] * val; b += L.rgb[2] * val;
      }
      // focal glow — the painting's brightest sliver, breathing outward
      const dx = u - fx; const dy = v - fy * (H * inv);
      const glow = Math.exp(-(dx * dx + dy * dy) * 14) * 0.72;
      r += pal.glow[0] * glow; gg += pal.glow[1] * glow; b += pal.glow[2] * glow;
...
type Drifter = { img: HTMLImageElement; x: number; y: number; size: number; speed: number; spin: number; rot: number };

export async function bootSkyTest(root: HTMLElement) {
  ...
  const paintings = await Promise.all(
    Array.from({ length: SKY_COUNT }, (_, i) => loadImg(`${ART}/skies/dark${i + 1}.jpg`)),
  );
  const squirrel = await Promise.all([1, 2, 3, 4].map((n) => loadImg(`${ART}/squirrel/flap-${n}.png`)));
  const planetImgs = (await Promise.all([0, 1, 2, 3, 4].map((n) => loadImg(`${ART}/planets/${n}.png`)))).filter(Boolean) as HTMLImageElement[];
  const debrisImgs = (await Promise.all([0, 1, 2].map((n) => loadImg(`${ART}/debris/${n}.png`)))).filter(Boolean) as HTMLImageElement[];
  const acornImg = await loadImg(`${ART}/acorn/1.png`);
  const palettes = paintings.map((p) => (p ? paletteFrom(p) : { base: [10, 14, 30], glow: [200, 210, 255], hues: [[90, 60, 160], [40, 90, 170], [160, 80, 120]] } as Palette));
  ...
  function envOf(s: number) { return s % SKY_COUNT; }
  function isProcedural(s: number) { return s % (SKY_COUNT * 2) >= SKY_COUNT; }

  function proceduralSky(env: number): HTMLCanvasElement {
  ...
  function spawnDrifter(offscreen: boolean) {
  ...
  for (let i = 0; i < 7; i++) spawnDrifter(false);
  ...
  window.addEventListener("keydown", (e) => { if (e.code === "Space" || e.code === "ArrowRight") advance(); });
  ...
  function frame(now: number) {
    ...
    if (proc) {
      ...
      if (img) {
        ...
        ctx.fillStyle = "#0a0e20"; ctx.fillRect(0, 0, cw, ch);
      }
    }
    ...
    for (let i = drifters.length - 1; i >= 0; i--) {
      ...
      p.x -= p.speed * dt;
      p.rot += p.spin * dt;
      if (p.x < -0.25) { drifters.splice(i, 1); spawnDrifter(true); continue; }
    ...
    if (acornImg) {
      for (const a of acorns) {
        a.x -= 0.06 * dt;
        if (a.x < -0.05) { a.x = 1.05 + rnd() * 0.3; a.y = 0.15 + rnd() * 0.7; }
        const size = 26 * dpr;
