...
## Modes entry relocation — 2026-08-24 current contract

Hyper Run is now entered from a full **HYPER RUN** row under **MODES → PROTOTYPES**, alongside Wormhole Run and using the same face/name/blurb/record-chip structure. The row is gated by `HYPER_RUN_ENABLED`, so it appears only on the beta page that can actually resolve and launch the experimental mission. It uses the scout ship as its face with the normal pilot as a safe art fallback and reports a saved best time when one exists.

The old Star Chart prototype card is removed. Its visibility used `BETA_FEATURES`, which is enabled on both root and beta pages, while `engine.flyLevel()` resolves the experimental mission only when `HYPER_RUN_ENABLED = IS_BETA`; that mismatch made the root-page card visible but unable to launch. The new Modes row opens the approved objective/control briefing over the hub, **BACK** returns to Modes, and **START RUN** routes `prototype-chapter-1` to `engine.flyLevel()`. Acceptance locks the row gate and formatting, absence from Star Chart, the exact launch ID, and the resulting `play` / READY / non-null fixed-step race world. Historical Log-placement sections below remain the record of earlier approved revisions, not the current navigation contract.
