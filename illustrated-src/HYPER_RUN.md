# Hyper Run - Course 1 design plan

**Status:** Phase 1 design proposal. No art or implementation is approved by this document. Phase 2 may begin only after explicit design sign-off; Phase 3 may begin only after separate art sign-off.

## Product promise

Hyper Run is a beta-only, fixed-course time trial inside the existing mission chart. The pilot threads ring gates to build speed and charge, punches through short hyperspeed corridors, returns to the same course farther ahead, and races to a finish line. Finish time alone awards stars. Acorns are a separate secondary record for pilots who can preserve a fast line while collecting them.

This is one new mission base, `race`, backed by course data. It is not an endless mode, not a new main-menu card, not a second campaign, and not a replacement for the existing `tunnel` missions.

## Course 1 scope

### Ships in Course 1

- One beta-only mission-chart entry, proposed as level `2-6`, renamed **HYPER RUN - COURSE 1** while beta is active. Slot `2-6` is deliberate: beta already uses `N-4` for Wormhole Run and `N-8` for Spill, so Course 1 can be reviewed without removing either test mission.
- One `race` mission base added to the existing `FlightBase` union and launched through the existing `flyLevel` route.
...
- Time goals evaluated by the existing goal/bitmask/`settleLevel` path.
- A race result-sheet variant in the existing level-complete overlay.
...
- More courses, procedural courses, randomized layouts, branching routes, shortcuts, laps, reverse courses, difficulty selection, or cross-course progression.
...
- Changes to live mission data. The `race` substitution exists only inside `IS_BETA`.

### Existing systems reused versus new

| Concern | Reused | New for Hyper Run |
| --- | --- | --- |
| Mission placement and unlock | `campaign.ts`, `LEVELS`, `IS_BETA`, `levelUnlocked` | One beta data substitution at `2-6`; `race` base; a Course 1 data record |
| Run lifecycle and banking | `flyLevel`, `resetRun`, `settleLevel`, star masks, acorn/XP/lifetime banking | Race timer and record fields passed through the same lifecycle |
...
Course 1 is a named constant rather than scattered conditionals. The intended shape is:
...
Course 1 uses seed `0x48595231` (`HYR1`), a canonical 360 x 640 physics space, length 36,000 course units, and at most three wormhole entries. The beta mission has these goals, in existing star-bit order:
...
The time goals are monotonic, so a three-star run necessarily earns the two-star bit in the same run. Acorns never participate in a star goal.

## Mechanical specification

### Locked Course 1 constants

| Rule | Course 1 value |
| --- | ---: |
| Canonical simulation | 360 x 640 at 60 fixed steps/second |
...
| Expected good-run cycles | 3; Course 1 maximum is 3 |
...
Course 1 has three charging legs followed by a final dash:
...
- After the third return, the meter remains active for visible ring feedback and speed gain, but Course 1's four remaining rings cannot fill it from empty.
...
| 0.30-0.78 s | Ring expands to 1.25x, rear aperture darkens into a painted mouth, space streaks bend inward. |
| 0.78-1.20 s | Pilot is pulled to the aperture center; normal objects stop participating; tunnel layers overtake the screen. |
| 1.20 s | Set `World.flight` to `tunnel`, consume charge, initialize the finite corridor with Course 1 seed plus cycle index, and unlock race controls. |
...
The existing tunnel host supplies seeded corridor nodes, wall bounds, region-layer rendering, and pickup collision. Course 1 configures it as a finite race stretch rather than a standalone Wormhole mission:
...
- Each stretch has exactly 20 collectible acorns on a seeded, replayable line. There are 12 authored acorns in normal space, making 72 the Course 1 maximum on a three-cycle run.
...
- Put the pilot at the same canonical vertical position reached in the tunnel, clamped to `y = 96.544`, with vertical velocity zero.
...
A good Course 1 run is expected to make three complete entry/tunnel/return cycles. A passive line makes zero; an average line is expected to make two.

### Debris

Debris exists in normal-space stretches only in Course 1 and reuses the illustrated debris art/circle collision helper. A swept-circle test is required at race speed.
...
- 1 star: finish Course 1.
- 2 stars: finish at or below 10,200 ticks / 2:50.000.
- 3 stars: finish at or below 8,700 ticks / 2:25.000.

Acorns are the secondary record. They never break a star threshold, subtract time, fill charge, or alter speed. Save data gains a backward-compatible optional record under the existing save object, without changing `SAVE_KEY` or legacy keys:

```ts
raceRecords?: Record<string, {
...
The Course 1 result sheet contains exactly:

1. `HYPER RUN � COURSE 1` kicker.
2. `FINISH` heading.
3. Finish time in `M:SS.mmm` derived from ticks.
4. `NEW BEST` when the run lowers `bestFinishTicks`; otherwise the delta behind best, such as `+4.217`.
5. Three star pips and the labels `FINISH`, `� 2:50.000`, `� 2:25.000`.
6. `ACORNS  n / 72` and `BEST  n / 72`; `NEW ACORN BEST` when applicable.
7. The existing newly-earned-star total and any unlock notice.
8. Existing navigation actions: `NEXT` when eligible, `FLY IT AGAIN`, and `STAR CHART`.

Course 1 does not show score, Flow, ring count, cycle count, debris hits, per-leg splits, or a leaderboard on the result sheet.

## Determinism: hard requirement

Course 1 is the same race for every pilot on every run. Two replays with the same ordered press/release ticks must produce bit-identical `finishTicks` and `runAcorns`.
...
- Run race physics in canonical 360 x 640 coordinates and scale only at draw/input boundaries. Phone aspect ratio, canvas resize, and device pixel ratio cannot change collisions or timing.
- Use the Course 1 seed plus the wormhole cycle index for all gameplay layout. Do not call `Math.random()` for ring, debris, acorn, corridor, or collision state.
...
2. The same replay rendered at 360 x 640 and at a tall-phone presentation size => identical authoritative result.
...
Any mismatch in finish ticks or acorns is a release blocker for Course 1.
...
All deliverable art is PNG, RGBA, transparent, with no baked background. Gate and portal plates are 256 x 256. Entry showpiece layers may be 512 x 512. Phase 2 mirrors byte-identical files into both art roots and includes a contact sheet for review; no art is wired into code before sign-off.
...
The contact sheet shows idle/passed/missed at 1x and game-size reduction, the four entry layers separately and composited at four timestamps, and the return portal separately and composited. Review checks: transparent edges, shared centers, aperture consistency, 360 x 640 gameplay readability, no state silhouette pop, palette contrast over representative dark skies, and exact mirrored-file hashes between art roots.
...
- Canvas backing resolution is capped at 2x CSS pixels for this mode. Reducing particle count or bloom resolution is allowed; reducing physics rate, collision fidelity, or authoritative timing is not.
...
- `illustrated-src/game/campaign.ts`: `race` base, time goal, Course 1 beta data.
...
- `illustrated-src/game/standalone.ts`: mission/result copy using the existing chart/sheet.
- Existing save type/source only for optional `raceRecords`; no save-key or legacy-key changes.
...
1. **Beta mission, not mode:** Hyper Run is proposed at beta level `2-6`; there is no main-menu entry. This follows the requested mission-system boundary and preserves existing Wormhole/Spill slots.
...
6. **No lethal race collisions:** Normal debris resets speed/charge; tunnel walls cost the acorn line. A time trial should resolve to a finish time, and Course 1's passive benchmark must remain measurable.
...
- **Phase 1 approval:** Agree on scope, constants, controls, time bands, mission slot, art inventory, and deviations in this document.
