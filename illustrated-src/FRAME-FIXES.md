...
- THE DIVE IS SHALLOW (owner: "shallow is the dive answer across the
  board"): the dive half of the range is halved before the pose curve, so
  in play a dive reaches `desc-1..3` and never `desc-4` or deeper. Rows
  marked † below are frames play no longer shows; they are lower priority
  and only matter if the dive is ever deepened again.
- Since this PR the loadout case rolls through the frames play reaches,
  both ways - `asc-8` down to `asc-1`, then `desc-1..3` and back - so every
  frame that flies can be checked in the case without flying. The rig
  editor (`docs/lab/rig/`, FRAMES) still shows every frame, † included.
...
3. Open the loadout case with the suit worn: the case sweeps the frames
   play reaches, both ways, 0.13s a frame, so a pop or a flash is visible
   without flying.
