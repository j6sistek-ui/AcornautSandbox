...
for(const [i,model] of manifest.models.entries()){
  for(const path of [model.file,model.atlas,...Object.values(model.banks).flat()].filter(Boolean))renderer.images.set(path,await loadImage(join(root,'docs/art',path)));
...
try{
  for(const path of ['/','/?viewer=1','/manifest.json','/core.mjs','/game/high-orbit.mjs','/art/suits/cosmic/parts.png']){const res=await fetch(url+path);assert.equal(res.status,200,path);assert.ok((await res.arrayBuffer()).byteLength>0);}
