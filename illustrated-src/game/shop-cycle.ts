import {
  BUNDLES, DUST_STICKER, HELMETS, PALS, SHOP_CYCLE, SUITS,
  bundleIds, isIap, wearsOwnHead,
} from "./catalog";
...
function dealFrom(pool: string[], count: number, seed: number): string[] {
  const items = [...pool];
...
export function selectShopCycle(day: number, owns: (id: string) => boolean) {
  day = Math.floor(day);
  const excluded = new Set<string>(SHOP_CYCLE.excludedIds);
  const excludedBundles = new Set<string>(SHOP_CYCLE.excludedBundleIds);
  ...
  const eligible = (id: string) => isIap(id) && !excluded.has(id) && !held.has(id) && !owns(id);
  ...
  const palMatches = (pal: string, kind: "suit" | "helm", ids: string[]) => BUNDLES.some(bundle =>
    bundle.items.some(item => item.kind === kind && ids.includes(item.id))
  ...
  const palSlots = Math.min(SHOP_CYCLE.pals,
    Math.max(0, gearSlots - Math.min(SHOP_CYCLE.minHelms, helmCandidates.length)));
