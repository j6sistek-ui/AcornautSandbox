...
export async function bootStandalone(root: HTMLElement) {
  ...
  function comboOf(cy: Cycle) {
    ...
    const trails = ids.map((i) => SET_TRAIL[i]).filter(Boolean) as string[];
  ...
  function drawShopBeta() {
    ...
    if (!combo.ids.length) {
      ...
      const names = combo.ids.map((i) =>
        SUITS.find((u) => u.id === i)?.name ?? HELMETS.find((h) => h.id === i)?.name ?? i);
      ...
      for (const it of items) {
        ...
        if (wearable) {
          b.onclick = () => {
            ...
            } else if (kind === "helm") tryOn = { ...tryOn, helm: it.id };
