import { xpCumulative, ART_VER, BETA_FEATURES, BUILD, ENVS, GAME_VERSION, GUIDE_HELM, GUIDE_SUIT, HELMETS, HELMET_SHELF, SUIT_SHELF, IAP_ITEMS, HYPER_RUN_ENABLED, IS_BETA, MOD_BATTERY_COST, MOD_SHIELD_COST, MODS, NEWS, PALS, PHYS, SUITS, TRACK, TRAILS, helmetWornBy, isIap, wearsOwnHead } from "./catalog";
 ...
 * fixed-step acceptance harness. Visibility remains gated by
 * HYPER_RUN_ENABLED at the Modes entry. */
...
export async function bootStandalone(root: HTMLElement) {
  ...
  function drawHome() {
    ...
    tile("t-modes", planet.c, "MODES", `${HYPER_RUN_ENABLED ? 6 : 5} ways to fly · Lab`,
    ...
    if (prototypeModeOpen && HYPER_RUN_ENABLED) {
  ...
  function drawModeSheet() {
    ...
    // Playable prototypes keep full mode rows and their real records. Lab
    // utilities below them remain dashed and deliberately quieter.
    if (HYPER_RUN_ENABLED) {
    ...
    if (engine.shopTab === "helmets") {
      ...
      for (const sec of HELMET_SHELF) {
        ...
        for (const h of items) {
          ...
          b.onclick = () => { if (!locked && open && (!premium || owned)) engine.buyHelmet(h.id); };
          row.append(b);
        }
        grid.append(row);
      }
      ...
      for (const t of TRAILS) {
        ...
        if (premium) b.classList.add("ac-premium");
        if (!open) b.classList.add("ac-cardoff");
        b.onclick = () => { if (open) engine.buyTrail(t.id); };
        grid.append(b);
      }
