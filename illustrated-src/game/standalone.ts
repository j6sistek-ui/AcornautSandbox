...
import { batteryUnlocked, dualPalUnlocked, equippedPals, deepUnlocked, helmetRevealed, lostUnlocked, palUnlocked, startShieldUnlocked, suitRevealed, iapOwned, modsUnlocked, starsOf, trailUnlocked, PILOT_NAME_MAX, boostReady, skipEligible, rewardOwned, ownsPremium} from "./save";
...
import { createDepotView, drawDepotWorkshop, drawSpillLaunchSetup, drawSpillStarters, drawSpillEnginePicker, spillUtilityArt } from "./spill-workshop";
...
export async function bootStandalone(root: HTMLElement) {
  ...
  function spillModuleIcon(id: SpillUtility) { return spillUtilityArt(id); }
  ...
  function drawPackSheet(id: string) {
    ...
    for (const [title, items] of groups) {
      ...
      for (const it of items) {
        ...
        if (ownsPremium(s, it.id)) chip.classList.add("owned");
        row.append(chip);
      }
      listWrap.append(row);
    }
