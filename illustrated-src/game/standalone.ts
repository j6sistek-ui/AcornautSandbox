...
import { SPILL_MODULE_MARKS, spillBuildFromState, spillBuildOre, spillPreviewState } from "./spill-presentation";
import { SPILL, SPILL_LEVELS, SPILL_SHOP, spillPrice, spillContractProgress, spillWaveSpec, restoreSpill, SPILL_MOD_INFO, type SpillBuyable, type SpillState, type SpillUpgrade } from "./spill";
...
export async function bootStandalone(root: HTMLElement) {
  ...
  const render = () => {
    ...
    if (snap.screen === "play") {
      ...
      if (sp) {
        if (sp.phase === "depot") {
          ...
          if (depot) depot.scrollTop = depotScroll;
          if (depotFocus) overlay.querySelector<HTMLElement>(`[data-spill-control="${depotFocus}"]`)?.focus({ preventScroll: true });
          return;
        }
    ...
    if (snap.screen === "dead" && snap.dead) {
      ...
      if (spill) {
        ...
        sheet.append(big);
        if (spill.cause === "GROUNDED") sheet.append(el("p", "ac-sub", "The last hull pip was lost at the lower boundary."));
        ...
        sheet.append(el("p", "ac-sub", `BUILD · Plating ${spill.up.plating} · Thrusters ${spill.up.thrusters} · Pulse ${spill.up.pulse}`),
          el("p", "ac-sub", spill.utilities.map(id => SPILL_UTILITIES[id].name).join(" + ") || "Stock utilities"),
          el("p", "ac-gold", mastery.current.title),
          el("p", "ac-sub", mastery.next ? `Next: clear wave ${mastery.next.at} for ${mastery.next.title} and a new ship signal.` : "Every ship signal earned. Push your personal record."));
  ...
  let depotTab: "ship" | "utilities" | "contracts" = "ship";
  let depotPart: SpillUpgrade | "shield" = "plating";
  let depotKey = "";
  ...
  // Stops are untimed; only the docking arm briefly disables the shelves.
  function drawSpillPrep() {
    const sp = engine.world.spill!;
    ...
    panel.append(el("p", "ac-sub", "Land at the Depot and choose one free system upgrade. Then survive endlessly, returning every five waves; wave 20 marks your first-pass victory."));
    ...
    for (const id of SPILL_UTILITY_IDS) {
      const u = SPILL_UTILITIES[id]; const option = el("option", "", `${u.name}${save.spillBest < u.unlock ? ` · clear wave ${u.unlock}` : " · earned"}`);
      option.value = id; option.disabled = save.spillBest < u.unlock; select.append(option);
    }
    select.value = sp.utilities[0] ?? "";
    select.onchange = () => engine.spillStarter((select.value || null) as SpillUtility | null);
    label.append(select); panel.append(label);
    panel.append(el("p", "ac-sub", `${mastery.current.title} · best wave ${save.spillBest}`
      + (mastery.next ? ` · next milestone ${mastery.next.at}` : " · all signals earned")));
    if (save.spillBest >= 5) {
      const signal = el("button", "ac-ghost", `${save.spillSignal ? "✓ " : ""}${mastery.current.finish}`);
      signal.onclick = () => engine.spillSignal(!save.spillSignal); panel.append(signal);
    }
    const launch = el("button", "ac-primary", "LAUNCH EXPEDITION"); launch.onclick = () => engine.spillLunge(); panel.append(launch);
    return panel;
  }

  function spillModuleIcon(id: SpillUtility) {
    const icon = document.createElementNS(SVG, "svg"); icon.setAttribute("viewBox", "0 0 24 24");
    icon.setAttribute("aria-hidden", "true"); icon.classList.add("ac-moduleglyph");
    for (const points of SPILL_MODULE_MARKS[id]) {
      const line = document.createElementNS(SVG, "polyline"); line.setAttribute("points", points.map(p => p.join(",")).join(" "));
      icon.append(line);
    }
    return icon;
  }
  ...
  function drawDepot(sp: SpillState) {
    ...
    if (key !== depotKey) { depotKey = key; depotTab = "ship"; depotPart = "plating"; }
    const wrap = el("div", "ac-lvlsheet ac-depotwrap");
    if (sp.phaseT < 0.1 && !engine.save.motionOff) wrap.classList.add("ac-depotenter");
    ...
    const title = el("div");
    title.append(el("p", "ac-kicker", sp.welcome ? "WELCOME ABOARD · PRE-FLIGHT" : `BAY ${Math.ceil(sp.wave / 5)} · WAVE ${sp.wave} CLEARED`),
      el("h2", "ac-lvlname", sp.welcome ? "Your first upgrade is free" : sp.firstPass ? "First pass complete" : "Salvage Depot"));
    const ore = el("div", "ac-depotore"); ore.append(el("img", "ac-depotcoin"), el("span", "", "ACORN COINS"), el("b", "", String(sp.ore)));
    (ore.querySelector("img") as HTMLImageElement).src = `${artRootUrl()}/pickups/acorn-coin.svg?v=${ART_VER}`;
    head.append(title, ore); sheet.append(head);
    ...
    const health = el("progress", "ac-depothealth"); health.max = sp.maxHull; health.value = sp.hull;
    health.setAttribute("aria-label", `Health: ${sp.hull} of ${sp.maxHull}`);
    const healthLabel = el("label", "", "HEALTH "); healthLabel.append(health);
    status.append(healthLabel, el("span", "", `SHIELDS ${sp.shield}`),
      el("span", "", sp.coreArmed ? "CORE ARMED" : "BAY SECURE · UNTIMED"));
    sheet.append(status);
    const tabs = el("div", "ac-depottabs");
    for (const [id, label] of [["ship", "UPGRADE SHIP"], ["utilities", `UTILITIES ${sp.utilities.length}/2`], ["contracts", "CONTRACTS"]] as const) {
      const b = el("button", depotTab === id ? "on" : "", label);
      b.disabled = !!sp.welcome && id !== "ship";
      b.dataset.spillControl = `tab-${id}`; b.setAttribute("aria-pressed", String(depotTab === id));
      b.onclick = () => { depotTab = id; render(); }; tabs.append(b);
    }
    sheet.append(tabs);
    const workspace = el("div", "ac-depotworkspace");
    const bay = el("div", "ac-depotbay");
    bay.append(el("p", "ac-depotguide", sp.welcome ? sp.freeUpgrade ? "CHOOSE YOUR FREE UPGRADE" : "UPGRADE FITTED · READY TO LAUNCH" : "SELECT A SYSTEM TO UPGRADE"));
    ...
    const lines = document.createElementNS(SVG, "svg"); lines.setAttribute("viewBox", "0 0 344 220"); lines.classList.add("ac-depotlinks");
    for (const [part, path] of [["plating", "M 50 44 L 95 70 L 151 115"], ["shield", "M 294 44 L 251 67 L 195 100"],
      ["thrusters", "M 50 178 L 65 145 L 99 116"], ["pulse", "M 294 178 L 279 144 L 246 118"]] as const) {
      const line = document.createElementNS(SVG, "path"); line.setAttribute("d", path);
      if (depotPart === part && depotTab === "ship") line.classList.add("on"); lines.append(line);
    }
    stage.append(lines);
    for (const part of ["plating", "shield", "thrusters", "pulse"] as const) {
      ...
      b.dataset.spillControl = `inspect-${part}`; b.setAttribute("aria-label", `Inspect ${SPILL_SHOP[part].name}, ${level} of ${total}`);
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (u) slot.append(spillModuleIcon(id));
      slot.append(el("span", "", u ? u.name : `＋ UTILITY SLOT ${i + 1}`));
      slot.disabled = !!sp.welcome;
      slot.onclick = () => { depotTab = "utilities"; render(); }; slots.append(slot);
    }
    bay.append(slots); workspace.append(bay);
    if (preview.ctx) {
      const ctx = preview.ctx;
      const previewBuild = spillBuildFromState(sp);
      let paintedAt = -Infinity;
      const tick = () => {
        if (!preview.c.isConnected) return;
        const now = performance.now();
        if (now - paintedAt < 1000 / 30 - 1) { requestAnimationFrame(tick); return; }
        paintedAt = now;
        const t = engine.save.motionOff ? 0 : now / 1000;
        ...
        if (flash > 0) {
          ...
          g.addColorStop(0, `rgba(117,234,197,${flash * 0.4})`); g.addColorStop(1, "rgba(117,234,197,0)");
          ctx.fillStyle = g; ctx.fillRect(0, 40, 344, 150);
        }
        paintShipPreview(ctx, engine.art, engine.save, 190, 113, 3.05, t, previewBuild);
    ...
    const buyButton = (what: SpillBuyable, label: string) => {
      ...
      b.append(el("b", "", label), el("span", "", price === null ? "COMPLETE" : price === 0 ? "FREE · FIRST UPGRADE" : `${price} COINS`));
    ...
    if (depotTab === "ship") {
      ...
      if (!sp.welcome && price !== null && price > sp.ore) consolePanel.append(el("p", "ac-depotneed", `Collect ${price - sp.ore} more Acorn Coins to fit this upgrade.`));
      if (part !== "shield" && tier >= 2) {
        ...
        for (const [id, spec] of Object.entries(SPILL_SPECIALTIES).filter(([, spec]) => spec.axis === part)) {
          ...
          const b = el("button", `ac-spilloption${selected ? " selected" : ""}`); b.dataset.spillControl = id; b.disabled = arming;
          b.append(el("b", "", `${selected ? "✓ " : ""}${spec.name}`), el("span", "", spec.desc), el("strong", "", "FREE REFIT"));
          b.onclick = () => { depotUpgradeAt = performance.now(); engine.spillSpecialize(id as SpillSpecialty); }; specs.append(b);
        }
        consolePanel.append(specs);
      } else if (part !== "shield") consolePanel.append(el("p", "ac-sub", "Tier II opens two free specializations for this system."));
    } else if (depotTab === "utilities") {
      consolePanel.append(el("p", "ac-kicker", "TWO MODULE SLOTS"), el("h3", "", "Outfit your ship"),
        el("p", "ac-sub", "Buy once this run. Unfit to swap; owned modules refit for free."));
      ...
      for (const id of SPILL_UTILITY_IDS) {
        ...
        const b = el("button", `ac-spilloption${fitted ? " selected" : ""}`); b.dataset.spillControl = id;
        ...
        b.append(spillModuleIcon(id), el("b", "", u.name), el("span", "", u.desc),
          el("strong", "", fitted ? "FITTED · UNFIT" : owned ? "FIT · OWNED" : `FIT · ${u.price} COINS`));
        b.onclick = () => { depotUpgradeAt = performance.now(); engine.spillUtility(id); }; utilities.append(b);
      }
      ...
      for (const line of forecast) consolePanel.append(el("p", "ac-depotforecast", line));
      if (sp.contract) consolePanel.append(el("p", "ac-gold", spillContractProgress(sp)));
      else for (const offer of spillContractOffers(sp.wave)) {
        const b = el("button", "ac-spilloption"); b.disabled = arming; b.dataset.spillControl = `contract-${offer.kind}`;
        b.append(el("b", "", `${offer.name} · +${offer.reward} COINS`), el("span", "", offer.desc));
        b.onclick = () => engine.spillContract(offer.kind); consolePanel.append(b);
      }
    }
    workspace.append(consolePanel); sheet.append(workspace);
    const services = el("div", "ac-depotservices");
    services.append(buyButton("repair", sp.hull < sp.maxHull ? "REPAIR HULL" : "HULL RESTORED"),
      buyButton("core", sp.coreBought ? sp.coreArmed ? "CORE ARMED" : "CORE SPENT" : "RESPAWN CORE"));
    if (!sp.welcome) sheet.append(services);
    if (sp.contractMessage) sheet.append(el("p", "ac-gold", sp.contractMessage));
    if (sp.depot?.bought.length) sheet.append(el("p", "ac-sub ac-depotreceipt", `Fitted: ${sp.depot.bought.map(b => SPILL_SHOP[b].name).join(" · ")}`));
    const footer = el("div", "ac-depotfooter");
    if (!sp.target) {
      const suspend = el("button", "ac-ghost", "SAVE & QUIT"); suspend.disabled = arming; suspend.onclick = () => engine.spillSuspend(); footer.append(suspend);
    }
    const go = el("button", "ac-primary", sp.welcome ? sp.freeUpgrade ? "CHOOSE AN UPGRADE TO LAUNCH" : "LAUNCH WAVE 1" : sp.firstPass ? "CONTINUE TO WAVE 21" : "BACK TO THE FIELD");
    go.disabled = arming || !!(sp.welcome && sp.freeUpgrade); go.onclick = () => engine.spillLeaveDepot(); footer.append(go);
    sheet.append(footer); wrap.append(sheet); return wrap;
  }
  ...
  const rewardKey = (r: {kind: string; id?: string; name?: string}) => `${r.kind}:${r.id ?? r.name}`;
  let pinCatalog: ReturnType<typeof buildPinCandidates> | undefined;
  function pinCandidates() { return pinCatalog ??= buildPinCandidates(); }
  function buildPinCandidates() {
    const rewards = STAR_REWARDS.filter(r => r.kind !== "stage").map(r => ({...r, cost: 0}));
    for (const [kind, list] of [["suit", SUITS], ["helmet", HELMETS], ["trail", TRAILS], ["pal", PALS]] as const) {
      for (const item of list) if (!isIap(item.id) && !rewards.some(r => r.kind === kind && r.id === item.id))
  ...
  function pinReward(kind: string, id?: string, name?: string) {
    const key = rewardKey({kind,id,name});
    if (!pinCandidates().some(r => rewardKey(r) === key)) return el("span");
  ...
  function pinnedHome() {
    const rail = el("section", "ac-pinnedhome"); rail.setAttribute("aria-label", "Pinned rewards");
    const save = engine.save;
    for (const key of pinnedKeys()) {
      ...
      const meter = el("progress"); meter.max = Math.max(1,target); meter.value = earned ? meter.max : Math.min(current,target);
      meter.setAttribute("aria-label", `${r.name} unlock progress`); open.append(meter);
      open.onclick = () => {
        const tab = ({suit:"suits",helmet:"helmets",trail:"trails",pal:"pals"} as const)[r.kind as "suit"|"helmet"|"trail"|"pal"];
        if (tab) { engine.setShopTab(tab); engine.open("hangar"); } else engine.open("log");
      };
      card.append(open, pinReward(r.kind,r.id,r.name)); rail.append(card);
    }
  ...
  function drawHangar() {
    ...
    // there, and the name, helmet, trail and pal ride its plate.
    {
      ...
      if (engine.shopTab === "ship") {
        const roman = (n: number) => (n ? "I".repeat(n) : "\u2013");
        ...
        plate.append(el("span", "ac-casesub",
          `Pulse ${roman(shipPick.pulse)} · Canopy ${roman(shipPick.shield)} · ${wornSuit.name} aboard`));
      } else {
    ...
    if (engine.shopTab === "helmets") {
      ...
      grid.append(fx);
      for (const p of PALS.filter((x) => !isIap(x.id) || iapOwned(s, x.id))) { const card = palCardOf(p); card.append(pinReward("pal",p.id,p.name)); grid.append(card); }
      ...
      if (STAR_MAP_PREVIEW) {
        ...
        look.append(el("p", "ac-kicker", "STAR MAP · APPEARANCE SAMPLE"),
          el("h3", "", "Rust Belt salvage kit"), el("p", "ac-sub", "Cosmetic finishes for the Spill ship. Your hull, modules and run remain the same."));
        const appearance = spillAppearance(s);
        for (const [kind, id, label] of [["finish","rust-runner","Rust Runner hull"],["trail","rust-wake","Rust Wake exhaust"]] as const) {
        ...
        concept.append(pal.c, el("b", "", "Rivet · placeholder concept"),
          el("p", "", "Tinbot artwork stands in for a future Spill-only cosmetic companion. No companion ability or equip option is added by this sample."));
        look.append(concept); grid.append(look);
      }
      const mastery = spillMastery(s.spillBest);
      const launch = el("section", "ac-shiplaunch");
      launch.append(el("p", "ac-kicker", "NEXT ENDLESS RUN"), el("h3", "", mastery.current.title),
        el("p", "ac-sub", "Land with a stock hull, choose one free system upgrade, and bring one earned utility on endless runs. Acorn Coins fund later upgrades; they are separate from your permanent acorns."));
      const starters = el("div", "ac-spilloptions");
      const stock = el("button", `ac-spilloption${!s.spillStarter ? " selected" : ""}`, "Stock ship · no utility");
      stock.dataset.shipStarter = "stock"; stock.setAttribute("aria-pressed", String(!s.spillStarter));
      stock.onclick = () => { shipPlan = null; engine.spillStarter(null); }; starters.append(stock);
      for (const id of SPILL_UTILITY_IDS) {
        const u = SPILL_UTILITIES[id], earned = s.spillBest >= u.unlock;
        const b = el("button", `ac-spilloption${s.spillStarter === id ? " selected" : ""}`);
        b.dataset.shipStarter = id; b.disabled = !earned; b.setAttribute("aria-pressed", String(s.spillStarter === id));
        b.append(spillModuleIcon(id), el("b", "", u.name), el("span", "", u.desc),
          el("strong", "", !earned ? `CLEAR WAVE ${u.unlock}` : s.spillStarter === id ? "EQUIPPED FOR LAUNCH" : "EQUIP FOR LAUNCH"));
        b.onclick = () => { shipPlan = null; engine.spillStarter(id); }; starters.append(b);
      }
      ...
      signal.disabled = s.spillBest < 5; signal.setAttribute("aria-pressed", String(s.spillSignal));
      signal.onclick = () => engine.spillSignal(!s.spillSignal); launch.append(signal); grid.append(launch);
      const plan = el("div", "ac-shipplanhead");
      plan.append(el("p", "ac-kicker", "DEPOT BUILD PREVIEW"), el("h3", "", "Plan your ship"),
        el("p", "ac-sub", "Inspect the actual hulls and systems below. Upgrade tiers are purchased during a run; previewing spends nothing."));
      ...
      plan.append(actions, el("p", "ac-shipreadout", `${previewShip.maxHull} HULL · ${previewShip.up.thrusters >= 2 ? 2 : 1} LUNGE CHARGE${previewShip.up.thrusters >= 2 ? "S" : ""} · ${previewShip.utilities.length}/2 UTILITIES`),
      ...
      grid.append(modules, el("p", "ac-shipnote", "DEPOT SERVICES · Full repair 30 Acorn Coins · Respawn Core 150 Acorn Coins, one extra life per run."));
  ...
  function drawLog() {
    ...
    const query = el("input"); query.placeholder = "Level or name"; query.setAttribute("aria-label", "Find a level by number, mission or zone name");
    const findButton = el("button", "ac-ghost", "Find"); findButton.type = "submit";
    const found = el("span", "ac-chart-found"); found.setAttribute("role", "status");
    find.onsubmit = event => {
      event.preventDefault(); const q = query.value.trim().toLowerCase();
      ...
      if (match) { goTo(match.id); found.textContent = `Level ${match.ord} · ${match.name}`; }
  ...
  function drawShopBeta() {
    ...
    if (!cart.ids.length) {
      ...
      t.append(el("b", "", "NOTHING SELECTED"),
        el("span", "", "Tap anything below to add it. Pick as many as you like."));
      bar.append(t);
    } else {
      const t = el("span", "ac-combotxt");
  ...
  function drawPackSheet(id: string) {
    const s = engine.save;
    const bn = BUNDLES.find((b) => b.id === id);
    if (!bn) return el("div", "");
    ...
    for (const [title, items] of groups) {
      ...
      for (const it of items) {
        ...
        if (helm) chip.append(helmCardOf(helm, 34));
        else if (title === "SUITS" && suit) chip.append(suitCardOf(suit, 34));
        else if (pal) {
          const { c, ctx } = miniCanvas(34, 34);
          if (ctx) paintPalPreview(ctx, engine.art, pal.id, 17, 17, 30);
          chip.append(c);
        } else {
          const t = TRAILS.find((x) => x.id === it.id);
          const { c, ctx } = miniCanvas(34, 34);
          if (ctx && t) paintTrailPreview(ctx, t, 17, 17, performance.now() / 1000);
          chip.append(c);
        }
        chip.append(el("span", "", it.name));
        if (iapOwned(s, it.id)) chip.classList.add("owned");
        row.append(chip);
      }
      listWrap.append(row);
    }
    ...
    if (confirmBuy) sheet.append(el("p", "ac-fine ac-mid",
      `This spends ${bn.dust.toLocaleString()} Star Dust. You have ${s.starDust.toLocaleString()}.`));
    wrap.append(sheet);
    wrap.onclick = (e) => { if (e.target === wrap) close(); };
    return wrap;
  }
  ...
  function drawHelp() {
    ...
    item(pic((ctx: CanvasRenderingContext2D, px: number) =>
      ...
      "COINS", "The Spill's currency. Spend it at the Depot.");
    item(pic((ctx, px) => {
