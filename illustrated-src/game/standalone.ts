...
import { dualPalUnlocked, equippedPals, deepUnlocked, helmetRevealed, lostUnlocked, palUnlocked, startShieldUnlocked, suitRevealed, starsOf, trailUnlocked, PILOT_NAME_MAX, boostReady, skipEligible, rewardOwned, ownsPremium} from "./save";
import { HYPER_RUN_MAX_ACORNS, HYPER_RUN_MISSION, STAR_REWARDS, STAR_UNLOCKS, countBits, fxText, goalText, levelUnlocked, starTitle, type LevelDef, type StarReward, RACE_GATES} from "./campaign";
...
export async function bootStandalone(root: HTMLElement) {
  ...
  spillControls.setAttribute("role", "group"); spillControls.setAttribute("aria-label", "Debris Field flight controls");
  ...
  let keptShopRails = new Map<string, { left: number; top: number }>();
  const keepShopRails = () => {
    keptShopRails = new Map([...overlay.querySelectorAll<HTMLElement>("[data-shop-rail]")]
      .map(r => [r.dataset.shopRail!, { left: r.scrollLeft, top: r.scrollTop }]));
  };
  const restoreShopRails = () => {
    for (const rail of overlay.querySelectorAll<HTMLElement>("[data-shop-rail]")) {
      const kept = keptShopRails.get(rail.dataset.shopRail!);
      if (kept) { rail.scrollLeft = kept.left; rail.scrollTop = kept.top; }
    }
  };
  const keepShelves = () => {
  ...
  for (const [kind, button] of [["dive", diveButton], ["thrust", thrust], ["lunge", lungeButton]] as const) {
  ...
  thrust.onpointerdown = e => {
    ...
    e.preventDefault(); engine.spillThrust(); updateSpillControls();
  };
  thrust.onkeydown = e => {
    ...
    if (e.repeat) return;
    engine.spillThrust(); updateSpillControls();
  };
  thrust.onclick = e => { if (e.detail === 0) { engine.spillThrust(); updateSpillControls(); } };
  ...
  const paint = () => {
    ...
    if (snap.screen !== "shop") { featureOpen = null; featureReview = null; confirmBuy = false; }
    if (snap.screen === "play") {
      ...
      if (sp) {
        if (sp.phase === "depot") {
          ...
          const depot = overlay.querySelector<HTMLElement>(".ac-depotcard");
          const guideChanged = oldGuide !== !!overlay.querySelector(".ac-depotguidecard");
          if (depot) {
            depot.scrollTop = guideChanged ? 0 : depotScroll;
            if (guideChanged) { depot.tabIndex = -1; depot.focus({ preventScroll: true }); }
            else if (depotFocus) overlay.querySelector<HTMLElement>(`[data-spill-control="${depotFocus}"]`)?.focus({ preventScroll: true });
          }
          return;
        }
    ...
    if (snap.screen === "pause") {
      ...
      if (engine.world.spill) {
        ...
        option("On-screen buttons", "Thrust, Dive and Lunge. Taps and swipes also work.", !engine.save.spillButtonsOff,
    ...
    if (snap.screen === "dead" && snap.dead) {
      ...
      if (spill) {
        ...
        sheet.append(big);
        if (spill.cause === "GROUNDED") sheet.append(el("p", "ac-sub", "You ran out of health at the lower edge."));
        ...
        sheet.append(el("p", "ac-sub", `LAST RUN · Health upgrades ${spill.up.plating} · Thrusters ${spill.up.thrusters} · Pulse ${spill.up.pulse}`),
          el("p", "ac-sub", spill.utilities.map(id => SPILL_UTILITIES[id].name).join(" + ") || "Stock utilities"),
          el("p", "ac-gold", mastery.current.title),
          el("p", "ac-sub", mastery.next ? `Next: clear wave ${mastery.next.at} for ${mastery.next.title} and a new engine color.` : "Every engine color earned. Beat your best wave."));
  ...
  function helpDot(painted = false) {
    const b = el("button", painted ? "ac-hub-sq" : "ac-helpdot");
    b.setAttribute("aria-label", painted ? "Help and controls" : "How to fly");
    b.append(painted ? hubIcon("help") : icon(I_HELP, 19));
  ...
  function acornImg(px: number) {
    ...
    const grad = (id: string, stops: [string, string][], x2 = "0", y2 = "1") => {
      const g = document.createElementNS(SVG, "linearGradient");
      g.setAttribute("id", id); g.setAttribute("x1", "0"); g.setAttribute("y1", "0"); g.setAttribute("x2", x2); g.setAttribute("y2", y2);
      for (const [off, col] of stops) { const st = document.createElementNS(SVG, "stop"); st.setAttribute("offset", off); st.setAttribute("stop-color", col); g.append(st); }
      defs.append(g);
    };
    grad(`${uid}b`, [["0", "#ffbe45"], ["0.55", "#f39a1c"], ["1", "#c9660c"]], "0.35", "1");
    grad(`${uid}c`, [["0", "#b0713a"], ["1", "#6a3a12"]]);
    svg.append(defs);
    const path = (d: string, fill: string, extra?: Record<string, string>) => {
  ...
  function costTag(n: number) {
    const w = el("span", "ac-costtag");
    w.append(acornImg(14), el("b", "", n.toLocaleString()));
    return w;
  }

  /** A Star Chart threshold uses the existing painted star, not a currency price. */
  function unlockTag(stars: number) {
    const tag = el("span", "ac-unlocktag");
    const star = boostArt("starunlock", 18);
    star.alt = "Stars";
    tag.append(star, el("span", "", `${stars.toLocaleString()} to Unlock`));
    return tag;
  }
  ...
  let spendAsk: { name: string; cost: number; run: () => string; card: HTMLElement; verb: string } | null = null;
  function spend(card: HTMLElement, name: string, cost: number, owned: boolean, run: () => string, verb = "unlock") {
    if (owned || cost <= 0 || engine.save.acorns < cost) return tx(card, run, cost);
  ...
  function drawSpendSheet() {
    ...
    sheet.setAttribute("role", "dialog"); sheet.setAttribute("aria-label", `Spend ${ask.cost} acorns to ${ask.verb} ${ask.name}?`);
    sheet.append(el("p", "ac-kicker", ask.verb.toUpperCase()));
    const q = el("p", "ac-spendask");
    q.append(el("span", "", "Spend "), acornImg(22), el("b", "", ask.cost.toLocaleString()),
      el("span", "", ` acorns to ${ask.verb} `), el("b", "", ask.name), el("span", "", "?"));
    sheet.append(q);
    const bal = el("p", "ac-sub ac-spendbal");
    bal.append(el("span", "", "You have "), acornImg(13), el("b", "", s.acorns.toLocaleString()),
      el("span", "", ` · ${(s.acorns - ask.cost).toLocaleString()} after`));
    sheet.append(bal);
  ...
  function suitRank(u: (typeof SUITS)[number]) {
    ...
    if (gate !== undefined) return 1_000_000 + gate;
    return u.cost;
  }
  function helmRank(h: (typeof HELMETS)[number]) {
    const s = engine.save;
  ...
  function headAside(acorns: number, paintedHelp = false) {
    const wrap = el("div", "ac-headaside");
    wrap.append(acornPill(acorns), dustPill(engine.save.starDust), helpDot(paintedHelp));
    return wrap;
  }
  ...
  function suitPitchDial(suitId: string) {
    ...
    const row = el("div", "ac-modes");
    (row as HTMLElement).style.gridTemplateColumns = "repeat(5, minmax(0,1fr))";
    for (const [label, d] of [["−5°", -5], ["−1°", -1], [`reset ${base}°`, 0], ["+1°", 1], ["+5°", 5]] as const) {
  ...
  function drawModeSheet() {
    ...
    const modeHead = header("FREE FLIGHT", "Modes");
    const back = modeHead.querySelector<HTMLButtonElement>(".ac-backbtn")!;
  ...
  function shopBundleBanner(bundle: (typeof BUNDLES)[number]) {
  ...
  function shopItemName(item: BundleItem) {
  ...
  function shopItemArt(item: BundleItem, px = 96) {
    ...
    if (item.kind === "suit") {
      ...
      if (suit) frame.append(suitCardOf(suit, px));
    } else if (item.kind === "helm") {
      const helm = HELMETS.find(h => h.id === item.id);
      if (helm) frame.append(helmCardOf(helm, px));
    } else {
      const { c, ctx } = miniCanvas(px, px);
      if (ctx && item.kind === "pal") paintPalPreview(ctx, engine.art, item.id, px / 2, px / 2, px * .72);
      else if (ctx) {
        const trail = TRAILS.find(t => t.id === item.id);
        if (trail) {
          ctx.save(); ctx.scale(px / 96, px / 96);
          paintTrailPreview(ctx, trail, 64, 48, 1); ctx.restore();
        }
      }
      frame.append(c);
    }
  ...
  function palCardOf(pl: (typeof PALS)[number], forShop = false) {
    ...
    b.append(c);
    const starGate = !premium && !open ? STAR_UNLOCKS.pals[pl.id] : undefined;
    if (starGate !== undefined) b.append(unlockTag(starGate));
  ...
  const DENY_TEXT: Record<string, (cost?: number, cur?: "acorns" | "dust") => string> = {
    poor: (c, cur) => {
  ...
  function drawHangar() {
    const s = engine.save;
    const shipPick: ShipPick = shipPlan ?? { plating: 0, thrusters: 0, pulse: 0, shield: 0,
    ...
    box.append(header("Suits & gear", "Loadout", headAside(s.acorns, true)));
    ...
    // the shop retains its separate display-case treatment.
    {
      ...
      if (!s.heroExpanded) stage.classList.add("ac-casecompact");
      let lastTap = 0;
      stage.addEventListener("pointerup", (e) => {
        const now = performance.now();
        if (now - lastTap < 320) { engine.setHeroExpanded(!engine.save.heroExpanded); lastTap = 0; }
        else lastTap = now;
        e.preventDefault();
      });
    ...
    if (engine.shopTab === "helmets") {
      ...
      const helmCard = (h: (typeof HELMETS)[number]) => {
          ...
          const starGate = !premium && !open ? STAR_UNLOCKS.helmets[h.id] : undefined;
          const helmState = premium ? (owned ? "OWNED" : "PREMIUM")
            : !open ? (starGate === undefined ? "LOCKED" : "")
            : owned ? "OWNED" : "";
          ...
          b.onclick = () => { if (!locked && open && (!premium || owned)) spend(b, h.name, h.cost, owned, () => engine.buyHelmet(h.id)); };
          if (open && (!premium || owned)) b.append(favStar(h.id));
          return b;
      };
      const helmListed = (h: (typeof HELMETS)[number]) =>
        !h.suitOnly && (!isIap(h.id) || ownsPremium(s, h.id));
      ...
      const suitCard = (u: (typeof SUITS)[number]) => {
        ...
        const starGate = !premium && !open ? STAR_UNLOCKS.suits[u.id] : undefined;
        b.append(
          suitCardOf(u, 88),
        ...
        b.onclick = () => { if (!premium || owned) spend(b, u.name, u.cost, owned, () => engine.buySuit(u.id)); };
        if (open && (!premium || owned)) b.append(favStar(u.id));
        return b;
      };
      const favSuits = favShelf(
        SUITS.filter((u) => !isIap(u.id) || ownsPremium(s, u.id)).map((u) => u.id),
      ...
      const trailCard = (t: (typeof TRAILS)[number]) => {
        ...
        const starGate = !premium && !open ? STAR_UNLOCKS.trails[t.id] : undefined;
        b.append(c, document.createTextNode(
          `${t.name}\n${open ? "OWNED"
            : premium ? "PREMIUM"
            : starGate === undefined ? "LOCKED" : ""}`));
        if (starGate !== undefined) b.append(unlockTag(starGate));
        if (premium) markPremium(b, t.colors[0]);
        if (!open || !compatible) b.classList.add("ac-cardoff");
        b.disabled = !compatible;
        if (!compatible) b.append(el("span", "ac-sub", t.id === "vanguardwake" ? "AcorNut only" : t.id === "arcflashwake" ? "Arcflash only" : "Change suit to wear"));
      ...
      if (STAR_MAP_PREVIEW) {
        ...
        look.append(el("p", "ac-kicker", "STAR MAP · APPEARANCE SAMPLE"),
          el("h3", "", "Rust Belt salvage kit"), el("p", "ac-sub", "Ship appearance only. Your upgrades and utilities stay fitted."));
        const appearance = spillAppearance(s);
        for (const [kind, id, label] of [["finish","rust-runner","Rust Runner ship finish"],["trail","rust-wake","Rust Wake exhaust"]] as const) {
          const b = el("button", "ac-ghost", `${appearance[kind] === id ? "✓ " : ""}${label}`);
          b.setAttribute("aria-pressed", String(appearance[kind] === id));
          b.onclick = () => engine.setSpillAppearance(kind, appearance[kind] === id ? "stock" : id); look.append(b);
        }
        const concept = el("div", "ac-sub");
        const pal = miniCanvas(64, 64);
        if (pal.ctx) paintPalPreview(pal.ctx, engine.art, "tinbot", 32, 32, 52);
        concept.append(pal.c, el("b", "", "Rivet · placeholder concept"),
          el("p", "", "Tinbot artwork stands in for a future Debris Field-only cosmetic companion. No companion ability or equip option is added by this sample."));
        look.append(concept); grid.append(look);
      }
      const launch = el("section", "ac-shiplaunch");
      launch.append(el("p", "ac-kicker", "YOUR NEXT SHIP"), el("h3", "", "Ready for a new run"),
        el("p", "ac-sub", "Start with 3 health and one free upgrade. Choose one earned utility below; later upgrades are bought during the run."),
        drawSpillStarters(engine, () => { shipPlan = null; }), drawSpillEnginePicker(engine));
      grid.append(launch);
      const plan = el("div", "ac-shipplanhead");
      plan.append(el("p", "ac-kicker", "DEPOT BUILD PREVIEW"), el("h3", "", "Plan your ship"),
        el("p", "ac-sub", "Inspect ship upgrades below. Upgrade tiers are purchased during a run; previewing spends nothing."));
      const actions = el("div", "ac-shipactions");
      const reset = el("button", "ac-ghost", "SHOW LAUNCH SHIP"); reset.onclick = () => { shipPlan = null; render(); }; actions.append(reset);
      const docked = restoreSpill(s.spillSuspended, 390, 760);
      if (docked) {
        const inspect = el("button", "ac-ghost", `VIEW SAVED BUILD · WAVE ${docked.wave}`);
        inspect.onclick = () => { shipPlan = spillBuildFromState(docked); render(); }; actions.append(inspect);
      }
      plan.append(actions, el("p", "ac-shipreadout", `${previewShip.maxHull} HEALTH · ${previewShip.up.thrusters >= 2 ? 2 : 1} DASH CHARGE${previewShip.up.thrusters >= 2 ? "S" : ""} · ${previewShip.utilities.length}/2 UTILITIES`),
        el("p", "ac-sub", `Build from stock: ${spillBuildOre(shipPick, s.spillStarter)} Acorn Coins · tier costs include preceding upgrades`));
      grid.append(plan);
      for (const axis of ["plating", "thrusters", "pulse", "shield"] as const) {
        ...
        for (let tier = 0; tier <= (isShield ? 2 : 3); tier++) {
          const pick: ShipPick = { ...shipPick, [axis]: tier, specialties: { ...shipPick.specialties } };
          if (!isShield && tier < 2) pick.specialties[axis] = null;
          const selected = shipPick[axis] === tier;
          const b = el("button", `ac-card ac-modcard ac-shipcard${selected ? " on" : ""}`);
          b.dataset.shipTier = `${axis}-${tier}`; b.setAttribute("aria-pressed", String(selected));
          const pic = miniCanvas(150, 82); pic.c.setAttribute("aria-hidden", "true");
          if (pic.ctx) paintShipPreview(pic.ctx, engine.art, s, 93, 44, 1.5, 0, pick);
          b.append(pic.c);
    ...
    sbTxt.append(el("b", "", "UNLOCK PREMIUM SETS"),
                 el("span", "", "Suits, helmets, trails and pals \u2014 sold in packs."));
  ...
  function rewardArt(item: { lvl?: number; kind: string; id?: string; name?: string }, px = 52) {
    const wrap = el("div", "ac-rewardwithpin");
    wrap.append(rewardArtPicture(item,px));
    return wrap;
  }
  function rewardArtPicture(
    item: { lvl?: number; kind: string; id?: string; name?: string },
    px = 52,
  ) {
    ...
    if (item.kind === "pal" && item.id) {
      ...
      g.addColorStop(0, "#e6dbff"); g.addColorStop(0.5, "#b494ff"); g.addColorStop(1, "#6d3fd1");
      ctx.fillStyle = g;
  ...
  function fullChart(stars: Record<string, number>, total: number) {
    ...
    levels.forEach((lvl, i) => {
      ...
      // are stars to show.
      if (mask) node.append(starPips(mask, "sm"));
    ...
    for (let gi = 0; gi < RACE_GATES.length; gi++) {
      ...
      for (let k = 0; k < COUNT; k++) {
        ...
        const { c, ctx } = miniCanvas(px, px);
        if (ctx && bank.length) drawSpriteOn(ctx, bank[spriteIdx] ?? null, px / 2, px / 2, px);
      ...
      if (blocking || done) tag.onclick = () => { hyperRunOpen = true; render(); };
      else tag.disabled = true;
      if (!done && !blocking && !arrived) {
        tag.setAttribute("aria-label",
          `Debris field after level ${g.after}. Reach level ${g.after} to attempt it.`);
      }
      if (blocking) tag.dataset.blockingBarrier = "true";
      map.append(tag);
    }
  ...
  function drawLog() {
    ...
    const goTo = (id?: string) => {
      const target = id ? [...box.querySelectorAll<HTMLElement>(".ac-mapnode")].find(n => n.dataset.level === id)
        : box.querySelector<HTMLElement>("[data-blocking-barrier], .ac-mapnode.cur");
    ...
    if (def.standalone) {
      ...
      if (g && !IS_BETA) {
        ...
        note.append(el("b", "", `DEBRIS FIELD AFTER LEVEL ${g.after}`),
                    el("span", "", ` \u00b7 finish in ${g.label} or faster to clear it`));
        sheet.append(note);
      }
  ...
  let featureReview: BundleItem | null = null; // an included product being reviewed
  ...
  /** Selection is shared with date/exhaustion tests; ownership stays save-aware here. */
  function shopCycle() {
    return selectShopCycle(shopDayIndex(), (id) => ownsPremium(engine.save, id));
  }
  ...
  function describeId(id: string) {
    ...
    if (u && h && !wearsOwnHead(u)) return `${u.name} with its helmet`;
    if (u) return u.name;
  ...
  function drawShopBeta() {
    ...
    const box = el("div", "ac-menu ac-shopbeta ac-shopvisual ac-shopcompact");
    ...
    if (tryOn.pal && !cy.pals.includes(tryOn.pal) && !cy.held.has(tryOn.pal) && !cy.owns(tryOn.pal)) {
      tryOn.pal = cy.pals[0] ?? "";
    }
    ...
    const CASE_W = 264;
    const CASE_H = 176;
    const stage = el("div", "ac-shopcase ac-shopviewer");
    ...
    if (ownHead) plate.append(el("span", "ac-tonohelm ac-casetag", fixedHeadTag(suit)));
    const choices = el("section", "ac-shopitems");
    const itemRail = el("div", "ac-shelfrow ac-shopitemrail");
    itemRail.dataset.shopRail = "items";
    itemRail.setAttribute("role", "group");
    itemRail.setAttribute("aria-label", "Today's items");
    choices.append(itemRail);
    stage.append(plate, pane, choices);
    ...
    if (heldNow.length) {
      ...
      t.append(el("b", "", "IN THE FEATURED PACK"),
        el("span", "", `${cy.feature?.name ?? "The pack"} — not sold separately today.`));
      const go = el("button", "ac-caseheldgo", "SEE THE PACK");
      go.onclick = () => {
        featureOpen = cy.feature?.id ?? null; featureReview = null; confirmBuy = false; render();
        overlay.querySelector<HTMLElement>(".ac-shopincluded button")?.focus({ preventScroll: true });
      };
      note.append(t, go);
      stage.append(note);
    }
    ...
    if (ctx) {
      ...
      const tick = () => {
        ...
        ctx.clearRect(0, 0, CASE_W, CASE_H);
        if (palDef) paintPalPreview(ctx, engine.art, palDef.id, CASE_W - 30, 43, 38);
        const low = lowSeatPal(engine.save, palDef?.id);
        if (low) paintPalPreview(ctx, engine.art, low, CASE_W - 30, 137, 38);
        paintFlightPreview(ctx, engine.art, suit, helm, 119, 96, 142, t, undefined, false, (suitPitchFor(engine.save, suit.id) * Math.PI) / 180);
        requestAnimationFrame(tick);
      };
      requestAnimationFrame(tick);
    }
    ...
    bar.dataset.focus = "shop:cart";
    bar.tabIndex = -1;
    if (!cart.ids.length) {
      bar.classList.add("ac-cartempty");
      const t = el("span", "ac-combotxt");
      t.append(el("b", "", "NOTHING SELECTED"),
        el("span", "", "Tap a card to add it."));
      bar.append(t);
    } else {
      const t = el("span", "ac-combotxt");
      ...
      clear.dataset.focus = "shop:clear";
      clear.onclick = () => {
        picked = new Set(); render();
        overlay.querySelector<HTMLElement>('[data-focus="shop:cart"]')?.focus({ preventScroll: true });
      };
      const go = el("button", "ac-primary ac-combobuy");
      go.dataset.focus = "shop:buy";
      go.setAttribute("aria-label", `Buy selected items for ${cart.dust.toLocaleString()} Star Dust`);
      ...
      go.onclick = () => {
        ...
        if (bought) {
          render();
          (overlay.querySelector<HTMLElement>('[data-focus="shop:buy"]')
            ?? overlay.querySelector<HTMLElement>('[data-focus="shop:cart"]'))?.focus({ preventScroll: true });
          if (refused) {
            announce(refused);
      ...
      bar.append(t, clear, go);
    }
    scroll.append(bar);

    // ---- THE SHELVES. Today's cycle, and only today's.
    const shelf = (kind: "suit" | "helm" | "pal", ids: string[]) => {
      ...
      for (const id of ids) {
        ...
        if (showing) b.classList.add("ac-previewing");
        b.onclick = () => {
          ...
          else if (kind === "suit") tryOn = { ...tryOn, suit: id };
          else {
            // A helmet needs a compatible preview pilot, without adding
            // that pilot to the cart or changing the equipped loadout.
            const selected = HELMETS.find(h => h.id === id);
            const shown = SUITS.find(u => u.id === tryOn.suit);
            const matching = selected?.suitOnly && SUITS.find(u => u.id === selected.suitOnly && !wearsOwnHead(u));
            const compatible = matching || (shown && !wearsOwnHead(shown) && !selected?.suitOnly ? shown : undefined)
              || SUITS.find(u => u.id === "flight" && !wearsOwnHead(u)) || SUITS.find(u => !wearsOwnHead(u));
            tryOn = { ...tryOn, suit: compatible?.id ?? tryOn.suit, helm: id };
          }
          render();
          overlay.querySelector<HTMLElement>(`[data-focus="shop:${kind}:${id}"]`)?.focus({ preventScroll: true });
        };
        row.append(b);
      }
    };
    ...
    for (const bn of [...(cy.feature ? [cy.feature] : []), ...cy.always]) {
      ...
      card.append(pr);
      if (quote.savings > 0) card.append(el("span", "ac-offersavings", `${quote.discountPercent}% OFF`));
      card.onclick = () => {
        featureOpen = bn.id; featureReview = null; confirmBuy = false; render();
        overlay.querySelector<HTMLElement>(".ac-shopincluded button")?.focus({ preventScroll: true });
      };
      scroll.append(card);
    }
    ...
    for (const id of BOOST_IDS) {
      ...
      row.append(boostArt(id, 34));
      const t = el("div", "ac-modtxt");
      const description = id === "levelskip" ? "3 stars instantly on any mission."
        : id === "starunlock" ? "Instant unlock any Star Chart reward" : spec.blurb;
      t.append(el("p", "ac-modname", spec.name), el("p", "ac-sub", description));
  ...
  function drawFeatureSheet(id: string) {
    ...
    const reviewItems: BundleItem[] = [...bn.items];
    const bonusWakes = new Map<string, string>();
    ...
    const counts = (["suit", "helm", "trail", "pal"] as const).map(kind => ({ kind, count: bn.items.filter(it => it.kind === kind).length }));
    ...
    if (quote.credit > 0) sheet.append(el("p", "ac-offercredit", `${quote.credit.toLocaleString()} Star Dust credit for owned items`));
    ...
    if (reviewing) {
      ...
      if (suit && wearsOwnHead(suit)) preview.append(el("p", "ac-fine", fixedHeadTag(suit)));
      if (bonusWakes.has(reviewing.id)) preview.append(el("p", "ac-fine",
        `Included with ${SUITS.find(u => u.id === bonusWakes.get(reviewing.id))?.name ?? "its suit"}`));
      if (reviewing.kind === "pal") preview.append(el("p", "ac-fine", PALS.find(p => p.id === reviewing.id)?.desc));
      sheet.append(preview);
      if (suit) engine.wantSuitArt(suit.id);
      if (reviewing.kind === "pal") engine.wantPalArt(reviewing.id);
      if (ctx) {
        const t0 = performance.now();
        ...
        const tick = () => {
          ...
          if (suit) paintFlightPreview(ctx, engine.art, suit, previewHelm, W / 2, H / 2, 158,
            t, undefined, false, (suitPitchFor(engine.save, suit.id) * Math.PI) / 180);
          else if (reviewing.kind === "pal") paintPalPreview(ctx, engine.art, reviewing.id, W / 2, H / 2, 124);
          else {
            const trail = TRAILS.find(x => x.id === reviewing.id);
            if (trail) paintTrailPreview(ctx, trail, W / 2 + 40, H / 2, t);
          }
      ...
      items.onclick = () => {
        featureReview = null; confirmBuy = false; render();
        overlay.querySelector<HTMLElement>(`[data-focus="review:${reviewing.kind}:${reviewing.id}"]`)?.focus({ preventScroll: true });
      };
      sheet.append(items);
    } else {
      ...
      for (const item of reviewItems) {
        ...
        b.dataset.shopItemId = item.id; b.dataset.shopItemKind = item.kind;
        if (item.kind === "trail" && bonusWakes.has(item.id)) b.dataset.bonusWake = "true";
        ...
        markPremium(b, glow);
        b.append(el("span", "ac-tilename", name), shopItemArt(item),
          el("span", owns(item.id) ? "ac-tileprice owned" : "ac-tileprice", status));
        b.onclick = () => {
          // Reviewing never equips, spends, or changes the separate single-item cart.
          featureReview = item; confirmBuy = false; render();
          overlay.querySelector<HTMLElement>(".ac-feature-reviewback")?.focus({ preventScroll: true });
        };
        row.append(b);
      }
    ...
    sheet.append(el("p", "ac-fine",
      premiumTrio ? "Signature wakes included · Also available individually"
    ...
    if (fullyOwned) {
      ...
      buy.append(el("span", "", due === 0 ? (confirmBuy ? "CONFIRM · COMPLETE BUNDLE · " : "COMPLETE BUNDLE · ")
        : confirmBuy ? "CONFIRM · SPEND " : premiumTrio ? "BUY TRIO · " : "BUY THE PACK · "),
        icon(I_DUST, 14, true), el("span", "", due.toLocaleString()));
      buy.onclick = () => {
        if (!confirmBuy) {
          confirmBuy = true; render();
          overlay.querySelector<HTMLButtonElement>(".ac-featurebuy")?.focus({ preventScroll: true });
          return;
        }
    ...
    sheet.append(buy);
    const close = () => {
      featureOpen = null; featureReview = null; confirmBuy = false; render();
      overlay.querySelector<HTMLElement>(`[data-focus="offer:${bn.id}"]`)?.focus({ preventScroll: true });
    };
    const back = el("button", "ac-ghost", "BACK");
  ...
  function drawCycleRoll(cy: Cycle) {
    ...
    const owned: string[] = [];
    const loginRewards: string[] = [];
    for (const id of IAP_ITEMS) {
      if (cy.owns(id)) owned.push(id);
      else if (cy.excluded.has(id)) loginRewards.push(id);
    ...
    const group = (title: string, ids: string[], why: string, showPrice = true) => {
      ...
      for (const id of ids) {
        ...
        if (showPrice) chip.append(el("i", "", `${idDust(id)}`));
        list.append(chip);
      }
      g.append(list);
      panel.append(g);
    ...
    group("LOGIN REWARDS", loginRewards, "Login reward suits; excluded from Shop rotation.", false);
  ...
  function drawDaily() {
    ...
    left.append(pips);
    left.append(el("p", "ac-sub", `Day ${st.streak} of ${DAILY_STREAK_LEN}. Come back tomorrow`));
    card.append(left);
  ...
  function drawProfile() {
    const s = engine.save;
    const box = el("div", "ac-menu ac-profile");
    ...
    if (editingName) {
      ...
      input.onkeydown = (e) => {
        ...
        else if (e.key === "Escape") { e.preventDefault(); editingName = false; render(); }
      };
    ...
    mtxt.append(el("b", "", "acornaut@outlook.com"),
                el("span", "", "Bugs, feedback and feature requests."));
  ...
  function drawHelp() {
    const box = el("div", "ac-menu ac-helpdeck");
    ...
    const modeSection = (id: string, title: string, theme: string) => {
      const section = el("section", `ac-helpmode ${theme}`);
      section.dataset.helpMode = id;
      const heading = el("h3", "ac-helpheading", title);
      heading.id = `ac-help-${id}`;
      section.setAttribute("aria-labelledby", heading.id);
      section.append(heading); scroll.append(section);
      return section;
    };
    const control = (glyph: string, title: string, action: string, keys: string, direction: "rise" | "dive") => {
      const card = el("div", `ac-helpcontrol ac-help-${direction}`);
      const mark = el("span", "ac-helpglyph", glyph); mark.setAttribute("aria-hidden", "true");
      card.append(mark, el("b", "", title), el("span", "ac-helpaction", action), el("small", "ac-helpkeys", keys));
      return card;
    };

    const normal = modeSection("normal", "Normal Mode", "m-fly");
    const controls = el("div", "ac-helpcontrols");
    controls.append(control("\u25B2", "TAP", "BOOST UP", "Space / \u2191", "rise"),
      control("\u25BC", "SWIPE DOWN", "DIVE", "\u2193 / S / Ctrl", "dive"));
    normal.append(controls);

    const debris = modeSection("spill", "Debris Field Mode", "m-spill");
    // Reuse the actual pad colors and input labels without the briefing's
    // explanatory paragraphs. The shared launch/briefing renderer stays intact.
    const spillHelp = drawSpillFlightHelp();
    const spillControls = spillHelp.querySelector<HTMLElement>(".ac-spillhelp-controls");
    if (spillControls) debris.append(spillControls);
    const extras = el("div", "ac-helpextras");
    const harderKick = spillHelp.querySelector<HTMLElement>(".ac-spillhelp-extra");
    if (harderKick) extras.append(harderKick);
    const briefing = el("button", "ac-ghost ac-helpbriefing", "BRIEFING");
    briefing.setAttribute("aria-label", "Debris Field briefing");
    briefing.dataset.spillBriefing = ""; briefing.onclick = openSpillHelp;
    extras.append(briefing); debris.append(extras);

    const hyper = modeSection("hyper", "Hyper Run Mode", "m-race");
    const hyperControls = el("div", "ac-helpcontrols");
    hyperControls.append(control("\u25B2", "HOLD TO RISE", "RELEASE TO FALL", "Hold Space / \u2191", "rise"),
      control("\u25BC", "DIVE", "SWIPE DOWN", "\u2193 / S / Ctrl", "dive"));
    const hyperExtras = el("div", "ac-helpextras");
    hyperExtras.append(el("small", "ac-helpkeys", "Double-tap + hold: boost"),
      el("small", "ac-helpkeys", "Tunnel: drag to steer"));
    hyper.append(hyperControls, hyperExtras);

    scroll.append(el("h3", "ac-helpheading ac-helpitemsheading", "Game Items"));
    ...
    item(pic((ctx, px) => {
      ...
      ctx.fillStyle = g; ctx.beginPath(); ctx.arc(px/2, px/2, px*0.46, 0, Math.PI*2); ctx.fill();
    }), "WORMHOLE", "Pulls you into a corridor. Tap to rise; gravity brings you down.");
  ...
  const closeTopSheet = () => {
    if (spillHelpOpen) { closeSpillHelp(); return true; }
      ...
      featureOpen = null; featureReview = null; confirmBuy = false;
    }
