...
export function addChartScenery(map: HTMLElement, levels: readonly LevelDef[], pos: Point[], step: number, art: ArtBank) {
  const layers: { node: HTMLElement; first: number; last: number; env: number }[] = [];
  let first = 0;
  while (first < levels.length) {
    const env = levels[first].fx.env ?? 0;
    let end = first + 1;
    while (end < levels.length && levels[end].fx.env === env) end++;
  ...
  const paint = () => {
    ...
    for (const layer of layers) {
      ...
      if (!near) { layer.node.replaceChildren(); layer.node.style.backgroundImage = "none"; continue; }
      if (layer.node.childElementCount) continue;
      ...
      const starLayer = document.createElement("div"); starLayer.className = "ac-zone-stars";
      const count = 30;
      for (let i = 0; i < count; i++) {
        const h = visualHash(`${zone.id}:star:${i}`);
        const star = document.createElement("i");
        star.style.left = `${h % 100}%`; star.style.top = `${(h >>> 8) % 100}%`;
    ...
    if (place && closest) place.textContent = ENVS[levels[closest.i].fx.env ?? 0].name;
  };
  const schedule = () => { if (!frame) frame = requestAnimationFrame(paint); };
  frame = requestAnimationFrame(() => {
    frame = 0;
    if (stopped) return;
    sc = map.closest<HTMLElement>(".ac-sheet-scroll");
    sc?.addEventListener("scroll", schedule, { passive: true });
    window.addEventListener("resize", schedule); map.addEventListener("focusin", schedule);
    paint();
  });
