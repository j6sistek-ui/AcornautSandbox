import { SKY_RGB, ENVS, PHYS, SUITS, TUT_ARM, TAP_ANIM_DURATION, helmetWornBy, skyIdFor, washScale, wearsOwnHead } from "./catalog.js?v=69";
import { drawTrailPreviewOn, drawPalOn, drawAstronautOn } from "./cosmetics.js?v=69";
import { drawSprite, skyImage, spriteHalo, SPRITE_HALO_PAD } from "./art.js?v=69";
import { retroBackdrop, retroPlanet, retroObstacle, retroAcorn, retroBlocker } from "./retro.js?v=69";
import { tunnelBoundsAt } from "./sim.js?v=69";
import { RACE_ACORNS, RACE_DEBRIS, RACE_HEIGHT, RACE_LENGTH, RACE_PILOT_X, RACE_RINGS, formatRaceTicks, raceTunnelAcorns, raceTunnelCenter, } from "./race.js?v=69";
...
function drawRaceSprite(ctx, art, id, x, y, size, tilt = 0) {
...
function drawRaceGateLayer(ctx, art, state, layer, x, y, size, tilt) {
    if (drawRaceSprite(ctx, art, `gate-${state}-${layer}`, x, y, size, tilt))
        return;
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(tilt);
    ...
    ctx.beginPath();
    if (layer === "back")
        ctx.ellipse(0, 0, size * 0.39, size * 0.46, 0, Math.PI * 0.08, Math.PI * 1.08);
    else
        ctx.ellipse(0, 0, size * 0.39, size * 0.46, 0, Math.PI * 1.08, Math.PI * 2.08);
    ctx.stroke();
    ctx.restore();
}
function drawRaceTunnel(ctx, w, art) {
    const race = w.race;
    const sx = w.W / 360;
...
function drawHyperRunWorld(ctx, w, save, art) {
    ...
    if (!tunnelView) {
        ...
        if (finishX < w.W + ringSize) {
            ...
            ctx.beginPath();
            ctx.moveTo(finishX, 80 * sy);
            ctx.lineTo(finishX, 560 * sy);
            ctx.stroke();
            ctx.setLineDash([]);
        }
    }
...
function paintIllustrated(ctx, spr, x, y, size, helmet, suit, _t = 0, art, frameKey = "idle-1", sprNext, keyNext, blend = 0, halo = "dark", tailRot = 0, tapAnimT = -1) {
