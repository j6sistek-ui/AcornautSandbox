...
function drawRays(ctx, W, H, env, weight) {
    if (!env.rays || weight <= 0.01)
        return;
...
export function retroObstacle(ctx, x, y, b) {
    const r = b.r;
    ...
    if (type === 'asteroid') {
        ...
        const panel = (x0) => {
            ...
            for (let i = 1; i < 4; i++) {
                ...
                ctx.beginPath();
                ctx.moveTo(x0 + 2, py);
                ctx.lineTo(x0 + r * 0.53, py);
                ctx.stroke();
            }
            for (let i = 1; i < 3; i++) {
                const px = x0 + i * r * 0.18;
                ctx.beginPath();
                ctx.moveTo(px, -r * 0.28);
                ctx.lineTo(px, r * 0.28);
                ctx.stroke();
            }
        };
        panel(-r * 1.1);
        ...
        ctx.lineWidth = 1.2;
        ctx.beginPath();
        ctx.arc(0, -r * 0.7, r * 0.22, -2.2, -0.6);
        ctx.stroke();
    }
    else if (type === 'ufo') {
        // hull
        ...
        ctx.fill();
        ctx.beginPath();
        ctx.arc(-r * 0.35, r * 0.28, r * 0.14, 0, Math.PI * 2);
        ctx.fill();
        // highlight
        ...
        ctx.lineWidth = 2.5;
        ctx.beginPath();
        ctx.ellipse(0, -r * 0.35, r * 0.72, r * 0.22, 0, 0, Math.PI * 2);
        ctx.stroke();
        ctx.beginPath();
        ctx.ellipse(0, r * 0.35, r * 0.72, r * 0.22, 0, 0, Math.PI * 2);
        ctx.stroke();
...
export function retroPlanet(ctx, x, y, r, kind0) {
    ...
    if (kind === 0) { // earth continents + clouds
        ...
        for (let gx = -4; gx <= 4; gx++) {
            for (let gy = -4; gy <= 4; gy++) {
                ...
                const py = gy * r * 0.15;
                if (px * px + py * py > r * r * 0.72)
                    continue;
                if ((gx * 7 + gy * 13 + 5) % 4 === 0)
                    continue; // dark blocks
        ...
        [[-0.7, -0.3, -0.2, -0.1, 0.15, -0.45], [-0.2, -0.1, 0.1, 0.25, 0.6, 0.1],
            ...
            for (let i = 2; i < seg.length; i += 2)
                ctx.lineTo(x + seg[i] * r, y + seg[i + 1] * r);
            ctx.stroke();
        });
...
export function retroAcorn(ctx, x, y, power) {
...
export function retroBackdrop(ctx, W, H, envA, envB, envBlend, stars) {
    ...
    const rgb = (key) => 'rgb(' + (mix(key, 0) | 0) + ',' + (mix(key, 1) | 0) + ',' + (mix(key, 2) | 0) + ')';
    const rgba = (key) => 'rgba(' + (mix(key, 0) | 0) + ',' + (mix(key, 1) | 0) + ',' + (mix(key, 2) | 0) +
        ',' + mix(key, 3).toFixed(3) + ')';
    const scalar = (key) => {
...
export function retroBlocker(env, debris, y) {
