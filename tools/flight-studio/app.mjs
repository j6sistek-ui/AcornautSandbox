...
function motionPanel(){return `<section class="control-section"><h2>Tap & recovery</h2><p class="intro">A tap starts a timed gesture. Pitch curves run from the tap through recovery; negative degrees point upward.</p><div class="fields">${field('Tap cycle · seconds','profile.tapSeconds',.1,5,.05,'8 frames over 1.0 s or 1.5 s: the duration is yours.')}${field('Tap progression','profile.tapEase',.2,4,.05,'Below 1 moves early; above 1 builds later.')}${field('Whole-model pitch · °','profile.basePitch',-90,90,1)}${field('Pitch settling · seconds','profile.pitchResponse',0,2,.01)}</div><div class="select-row" style="margin-top:20px">${select('When another tap arrives','profile.retrigger',[['restart','Restart the tap cycle'],['continue','Finish current cycle'],['queue','Queue one following cycle']])}</div>${check('Finish the tap cycle before entering descent','profile.finishTap')}</section>`+
...
function partsPanel(){if(model.family==='bank')return `<section class="control-section"><h2>This model uses painted frames</h2><p class="intro">Tune its timing, sequence and pitch in Frame banks. Individual joints require a cut-parts rig.</p><button data-action="frames">Open frame banks</button></section>`;
...
function frameCells(kind){const p=project.profile,order=kind==='tap'?p.tapOrder:p.descOrder,bank=kind==='tap'?p.tapSource:'desc';
  return `<div class="frame-strip">${order.map((index,slot)=>{const path=model.banks[bank]?.[index]||model.file;return `<div class="frame-cell" data-slot="${slot}" data-bank="${kind}"><span>${slot+1} / source ${index+1}</span><img alt="${kind} source frame ${index+1}" src="/art/${path}"><div class="row"><button aria-label="Move ${kind} frame ${slot+1} earlier" data-move="${kind}:${slot}:-1" ${slot===0?'disabled':''}>←</button><button aria-label="Move ${kind} frame ${slot+1} later" data-move="${kind}:${slot}:1" ${slot===order.length-1?'disabled':''}>→</button></div>${kind==='tap'?`<label>Hold weight<input type="number" aria-label="Tap frame ${slot+1} hold" data-path="profile.tapWeights.${slot}" min=".1" max="10" step=".1" value="${p.tapWeights[slot]}"></label>`:''}<label>Pitch offset °<input type="number" aria-label="${kind} frame ${slot+1} pitch" data-path="profile.${kind==='tap'?'tapOffsets':'descOffsets'}.${slot}" min="-90" max="90" step="1" value="${p[kind==='tap'?'tapOffsets':'descOffsets'][slot]}"></label></div>`;}).join('')}</div>`;
}
function framesPanel(){if(model.family!=='bank')return `<section class="control-section"><h2>This model uses a cut-parts rig</h2><p class="intro">There are no painted motion frames to reorder. Tune the articulated body in Body parts.</p><button data-action="parts">Open body parts</button></section>`;
  const options=[['tap','Tap bank'],['asc','Ascent paintings, played on tap'],['loop','Continuous loop'],['still','Still portrait']].filter(([k])=>k==='still'||model.banks[k].length);
...
function renderControls(){
  ...
  document.querySelectorAll('nav button').forEach(b=>b.classList.toggle('active',b.dataset.panel===panel));drawCurves();
}
function updateLabels(){
  $('model-label').textContent=model.name;$('viewer-model').textContent=model.name;
  $('family-badge').textContent=model.family==='bank'?'FRAME BANK':'CUT RIG';
...
function draw(){
  ...
  if(isViewer){const {w,h,d}=fit(flight);flightCtx.setTransform(d,0,0,d,0,0);paintBackground(flightCtx,w,h,time,project.view.background,true);
    ...
    const a=simulation.animation;$('stage-label').textContent=ready?(model.family==='bank'?a.stage+' · '+a.bank+' '+(a.frame+1):a.stage+' · fixed part scale'):'Loading artwork…';
...
$('model').innerHTML=manifest.models.map(m=>`<option value="${m.id}">${html(m.name)}${m.family!=='bank'?' · Rig':''}</option>`).join('');$('model').onchange=()=>choose($('model').value);
